...
def validate_installation_target(project_root: Path, self_install_requested: bool = False) -> None:
    ...
    """
    # 1. Check not inside an existing hooks-daemon installation. Keyed on a
    # REAL clone (_project_root_is_daemon_repo), not merely on the directory
    ...
    for parent in project_root.parents:
        ...
        if candidate.exists() and _project_root_is_daemon_repo(candidate):
            raise InstallationError(
...
def copy_init_script(project_root: Path, daemon_dir: Path, self_install: bool = False) -> None:
    ...
    if self_install:
        # For self-installation, create symlink to avoid duplication. The
        # target MUST be repo-relative, not absolute: an absolute symlink
        # bakes in this machine's checkout path (e.g. /workspace/init.sh)
        # and dangles on every other clone (MT-1, Plan 00198).
...
def create_settings_json(project_root: Path) -> None:
    ...
    # Plan 00102 Tier 1: invoke each hook wrapper via `bash <abs-path>` so the
