...
from claude_code_hooks_daemon.plan_qa.checks.stats_arithmetic import (
    CHECK_ID as STATS_ARITHMETIC_CHECK_ID,
)
from claude_code_hooks_daemon.plan_qa.checks.stats_arithmetic import (
    REMEDIATION as STATS_ARITHMETIC_REMEDIATION,
)
from claude_code_hooks_daemon.plan_qa.checks.stats_arithmetic import (
...
RULE_PLAN_STATS_ARITHMETIC: Final[str] = STATS_ARITHMETIC_CHECK_ID
...
# The plan index's reconciliation bullet must agree with its own closing
# self-check. The implementation is plan QA's `plan-stats-arithmetic` check
# (`plan_qa/checks/stats_arithmetic.py`), which also runs in the commit gate;
# this rule calls it rather than carrying a second copy (Plan 00466 N13).
...
# The ONE thing under a plan that is ignored on purpose. Plans keep probe
# scripts, and importing one writes bytecode beside it -- gitignored,
# untracked, and therefore inside this rule's set without being a document at
# all. The remediation the rule offers (anchor the pattern so the file gets
# tracked) would be actively wrong for bytecode. Deliberately narrow: an
# ignored ``.py`` probe IS a silent loss and stays flagged.
_BYTECODE_CACHE_DIRNAME: Final[str] = "__pycache__"
_BYTECODE_SUFFIXES: Final[tuple[str, ...]] = (".pyc", ".pyo")
...
def ignored_plan_documents(root: Path) -> tuple[str, ...]:
    ...
    Python bytecode is filtered out (:func:`_is_bytecode_cache`): it is
    generated rather than authored, so "ignored" is the correct state for it
    and the rule's own remediation would be wrong.
    ...
    return tuple(
        sorted(path for path in result.stdout.split(_NUL) if path and not _is_bytecode_cache(path))
    )


def _is_bytecode_cache(rel_path: str) -> bool:
    """True for CPython's own generated bytecode, wherever it landed."""
    parts = rel_path.split("/")
    return _BYTECODE_CACHE_DIRNAME in parts[:-1] or parts[-1].endswith(_BYTECODE_SUFFIXES)
...
_REMEDIATION_PLAN_STATS_ARITHMETIC: Final[str] = STATS_ARITHMETIC_REMEDIATION
...
def plan_stats_arithmetic(root: Path) -> tuple[tuple[str, str], ...]:
    ...
    A thin adapter over plan QA's :func:`stats_arithmetic_disagreements`, the
    one implementation. An index without the bullet (every client project)
    reports nothing.
    ...
    if not index.is_file():
        return ()
    return tuple(
        (_PLAN_INDEX_PATH, disagreement.message)
        for disagreement in stats_arithmetic_disagreements(index.read_text(encoding="utf-8"))
    )
