...
#
_vb_watchdog() {
    local owner="$1" deadline="$2" probe out identity current now
    ...
    while probe="$(kill -0 "$owner" 2>&1)"; do
        ...
        if [ "$now" -ge "$deadline" ]; then
            # Plan 00466 N59: a live pid at the bound is not proof on its own
            # -- a build KILLed just after one poll frees its pid for reuse
            # before the next, so the TERM goes only to a pid that still
            # carries the identity recorded at watchdog start.
