...
if [ $STATUS_EXIT_CODE -eq 0 ]; then
    ...
    echo ""
    echo -e "${BOLD}OPTION 1: Replace with default config (RECOMMENDED)${NC}"
    echo ""
    echo "  Default config: $HOOKS_DAEMON_DIR/.claude/hooks-daemon.yaml.example"
    echo "  Your config:    $PROJECT_ROOT/.claude/hooks-daemon.yaml"
    echo "  Backup:         ${CONFIG_BACKUP:-unknown}"
    echo ""
    echo "  To replace:"
    echo "    cp $HOOKS_DAEMON_DIR/.claude/hooks-daemon.yaml.example $PROJECT_ROOT/.claude/hooks-daemon.yaml"
    echo "    $VENV_PYTHON -m claude_code_hooks_daemon.daemon.cli restart"
    echo ""
    echo -e "${BOLD}OPTION 2: Fix specific validation errors (for custom handlers)${NC}"
    echo ""
    echo "  Read release notes: $HOOKS_DAEMON_DIR/RELEASES/v${NEW_VERSION}.md"
    echo "  Edit: $PROJECT_ROOT/.claude/hooks-daemon.yaml (remove deprecated settings)"
...
if [ -n "$RECOMMENDATIONS" ]; then
    ...
    echo ""
    echo "To enable recommended handlers:"
    echo "  Edit: $PROJECT_ROOT/.claude/hooks-daemon.yaml"
    echo "  Set: handlers.<event_type>.<handler_name>.enabled: true"
    echo "  Restart: $VENV_PYTHON -m claude_code_hooks_daemon.daemon.cli restart"
    echo ""
fi
