...
- **🎯 NEW: /hooks-daemon User Skill** - Single skill with 7 subcommands (upgrade, health, dev-handlers, logs, status, restart, handlers) deployed to user projects
- **🔄 Breaking Changes Lifecycle** - Complete infrastructure preventing upgrade issues with historical guides, automated detection, and smart validation
- **🚨 CRITICAL BUG FIX: Plugin Handler Suffix** - Fixed silent failure of plugin handlers with "Handler" suffix
- **⚡ FAIL FAST Enforcement** - Project handlers (TIER 1) always crash on errors; no silent failures
- **📚 Comprehensive Documentation** - 1,600-line Plan Workflow guide, separate Plans vs Workflows docs
- **✅ Test Coverage: 95.2%** - Exceeded minimum requirement with comprehensive improvements
- **📊 Effort Level Signal Bars** - Status line now displays ▌▌▌ effort bars matching Claude Code's UI
...
**Problem Solved:** Users upgrading v2.10.1 → v2.13.0 encountered unknown handler errors with no upgrade guidance. Handlers were removed in v2.11.0 and renamed in v2.12.0 without migration documentation.

**Solution Components:**

1. **Historical Upgrade Guides** (Phase 1):

   - v2.10→v2.11: Handler removals (validate_sitemap, remind_validator)
   - v2.11→v2.12: Handler rename (validate_eslint_on_write → lint_on_edit)
   - v2.12→v2.13: Safe upgrade (no breaking changes)
...
01. Pre-Release Validation
02. Version Detection
03. Version Updates
04. Changelog Generation
05. Release Notes Creation
06. Breaking Changes Detection
07. Upgrade Guide Generation
08. Upgrade Guide Verification Gate (BLOCKING)
09. QA Verification Gate (BLOCKING)
...
Added visual effort level indicator to the status line using ▌▌▌ bars matching Claude Code's own UI.

**Features:**

- Bars display for all models (active = orange, inactive = grey)
- Enhanced display for Claude 4+ models
- Uses exact ▌▌▌ character matching Claude Code's effort UI
...
- server.py: Use handle_tier2_error() in \_get_input_validator() (8 lines → 6 lines)
...
- **test_upgrade_compatibility.py**: Fixed import error `constants.event` → `constants.events` (commit d0094ce)

#### Effort Level Signal Bars Styling

- Fixed bar colours to orange (active) / grey (inactive) matching Claude Code UI (commit 95a6b4d)
- Fixed bar character to ▌▌▌ matching Claude Code's actual effort UI (commit 0b031b1)
