...
- **`remind_validator`** (SubagentStop) - Removed in v2.11.0
  - **Why**: Project-specific reminder that doesn't belong in core daemon. This handler reminded users to run validator agents after builder agents completed, which is specific to one project's builder/validator agent pattern.
  - **Migration**: Remove from config, or migrate to project-level handlers if needed. See upgrade guide for recreation example.
  - **Guide**: [v2.10-to-v2.11 Upgrade Guide](../CLAUDE/UPGRADES/v2/v2.10-to-v2.11/v2.10-to-v2.11.md)

**Impact**: Low - These handlers were project-specific and unlikely to be used by other projects. If your config references them, they will be safely ignored (daemon continues to work). The proper place for project-specific handlers is `.claude/project-handlers/`.
