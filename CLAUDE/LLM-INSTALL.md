...
| `.claude/agents/hooks-daemon-plan-dedupe-scout.md` | Plan duplicate-check sub-agent (plan workflow only) | Claude Code discovers sub-agents only under `.claude/agents/`; the `hooks-daemon-` prefix keeps it from colliding with your own agents, since a name clash there silently drops one definition instead of erroring. To customise it, copy it under a name of your own rather than editing it — this copy is refreshed on every upgrade |
