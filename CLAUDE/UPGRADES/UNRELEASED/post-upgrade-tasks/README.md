...
| `05-review-new-denials-from-strict-mode-and-safety-guards.md`               | workflow-change  | notification | Any project with `daemon.strict_mode: true`, or any `SAFETY`+`BLOCKING` handler         | `strict_mode` and a raising `SAFETY`+`BLOCKING` handler now actually deny; review any newly-surfaced denials.                    |
