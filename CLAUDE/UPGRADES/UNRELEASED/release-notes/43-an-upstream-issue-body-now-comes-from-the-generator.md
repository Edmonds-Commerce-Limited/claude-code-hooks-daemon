...
- `--web`. It files nothing — it opens GitHub's own issue forms, which state
  the same rule, and the defect form cannot be submitted without ticking two
  acknowledgements. That is the fallback when the generator genuinely cannot
  run, and it is a deliberate hole: a gate whose only escape is evasion teaches
  evasion.
