...
| 1.2 read how others decide when to speak | Done. `deployed_artefact_drift` speaks every session while drift lasts, keyed on presence. It has no once-only cache. `gitignore_safety_checker` keeps a content-hash JSON cache under the daemon's untracked dir. The once-only key reuses that shape (`secret_file_absence_cache.json`).                                                                                                             |
...
- A disabled declaring handler, an unconfigured default, a glob, a bare name,
  or an unloadable config: all silent.
- A failed cache write still reports, and repeats next session, which is the
  safe direction (`test_unwritable_cache_still_reports_and_never_raises`).
...
- `scripts/qa/error_hiding_exclusions.json`: `_load_config`,
  `_read_absence_hash` and `_write_absence_hash`, modelled on the
  `gitignore_safety_checker` entries. `_load_config` logs at WARNING.
