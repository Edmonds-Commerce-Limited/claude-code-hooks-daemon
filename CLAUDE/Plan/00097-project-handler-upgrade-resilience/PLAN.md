...
- [ ] **Task 1.1**: Add abstract method detection helper
  - [ ] Add `_ABSTRACT_METHOD_VERSIONS` dict constant to `project_loader.py`
  - [ ] Add `_get_missing_abstract_methods(cls)` helper that returns list of `(method_name, version)` tuples for any abstract methods not overridden
  - [ ] Write tests for helper: returns empty list for concrete class, returns methods for abstract class
  - TDD: write tests first in `tests/unit/handlers/test_project_loader.py`

- [ ] **Task 1.2**: Improve error message when no concrete handler found
  - [ ] In `load_handler_from_file()`, before the "No Handler subclass found" raise, scan handler-shaped classes (subclasses of Handler, even abstract ones) and call `_get_missing_abstract_methods()`
  - [ ] If missing methods found: raise with `"HandlerClass is missing abstract methods introduced in: get_claude_md (v2.30.0). Add: def get_claude_md(self) -> str | None: return None — see CLAUDE/UPGRADES/v2/v2.29-to-v2.30/"`
  - [ ] If no abstract subclasses either: keep existing "No Handler subclass found" message
  - [ ] Tests: missing get_claude_md produces version-specific message; class with all methods produces no such message

- [ ] **Task 1.3**: Make discover_handlers() skip broken handlers (don't crash)
  - [ ] Wrap `load_handler_from_file(py_file)` call in try/except RuntimeError in `discover_handlers()`
  - [ ] On failure: log structured warning with filename + full error message; continue to next file
  - [ ] Collect failures into a list; after loop, if any failures: log summary "N project handler(s) failed to load; daemon started with remaining handlers"
  - [ ] Tests: a directory with one broken and one working handler loads the working one and logs a warning for the broken one

- [ ] **Task 1.4**: Run QA and daemon restart
  - [ ] `./scripts/qa/run_all.sh` — all checks must pass
  - [ ] `$PYTHON -m claude_code_hooks_daemon.daemon.cli restart && $PYTHON -m claude_code_hooks_daemon.daemon.cli status`

- [ ] **Task 1.5**: Checkpoint commit
  - [ ] `git add` specific files, commit `Fix: project handler load failures no longer crash daemon`

### Phase 2: Fix validate-project-handlers CLI command

- [ ] **Task 2.1**: Exit non-zero on failures
  - [ ] In `cmd_validate_project_handlers()` (`cli.py`), track a `has_failures` boolean
  - [ ] When a handler fails to load (RuntimeError), set `has_failures = True`
  - [ ] Return `1` from the command function if `has_failures` is True
  - [ ] Tests: command returns 1 when a broken handler exists

- [ ] **Task 2.2**: Surface version-specific guidance in CLI output
  - [ ] When failure message contains a version string (e.g. "v2.30.0"), print an additional line: `→ Upgrade guide: CLAUDE/UPGRADES/v2/v2.29-to-v2.30/`
  - [ ] This should be a general pattern — parse the version from the error message and construct the upgrade guide path
  - [ ] Tests: output includes upgrade guide path when version is in error message

- [ ] **Task 2.3**: Run QA and daemon restart
  - [ ] `./scripts/qa/run_all.sh`
  - [ ] Daemon restart verification

- [ ] **Task 2.4**: Checkpoint commit
  - [ ] `git add` specific files, commit `Fix: validate-project-handlers exits non-zero and shows upgrade guide on failure`

### Phase 3: Create upgrade guide for v2.29→v2.30

- [ ] **Task 3.1**: Create upgrade guide directory and files
  - [ ] `mkdir -p CLAUDE/UPGRADES/v2/v2.29-to-v2.30/`
  - [ ] Create `CLAUDE/UPGRADES/v2/v2.29-to-v2.30/README.md` following the template at `CLAUDE/UPGRADES/upgrade-template/README.md`
  - [ ] Content must cover:
    ...
    - Verification steps
  - [ ] Create `CLAUDE/UPGRADES/v2/v2.29-to-v2.30/verification.sh` that runs validate-project-handlers and checks exit code

- [ ] **Task 3.2**: Update RELEASES/v2.30.0.md to reference upgrade guide
  - [ ] Add `⚠️ BREAKING CHANGES` section if not present
  - [ ] Add link to `CLAUDE/UPGRADES/v2/v2.29-to-v2.30/`

- [ ] **Task 3.3**: Checkpoint commit
  - [ ] Commit `Add: upgrade guide v2.29→v2.30 for get_claude_md() breaking change`

### Phase 4: Update release process to prevent recurrence

- [ ] **Task 4.1**: Add Handler ABC checklist to RELEASING.md Step 6.5
  - [ ] In `CLAUDE/development/RELEASING.md`, in the "Breaking Changes Check" section, add a specific check:
  ...
  - [ ] Also add: before publishing, verify `_ABSTRACT_METHOD_VERSIONS` in `project_loader.py` is updated to include the new method

- [ ] **Task 4.2**: Update `_ABSTRACT_METHOD_VERSIONS` to be accurate
  - [ ] Verify `get_acceptance_tests` version in git log
  - [ ] Ensure all current abstract methods are in the registry

- [ ] **Task 4.3**: Final QA run and daemon restart
  - [ ] `./scripts/qa/run_all.sh`
  - [ ] Daemon restart + status check

- [ ] **Task 4.4**: Final checkpoint commit
  - [ ] Commit `Add: Handler ABC version registry and release process checklist`
