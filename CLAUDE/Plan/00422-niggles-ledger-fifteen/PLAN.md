...
| N6  | a worktree cannot run the acceptance gates, and says the wrong reason    | Plan 00424                                                         | 🔄 Fault 1 closed (remedies 1, 2 done; 3 declined); fault 2 DIAGNOSED — not a worktree fault at all: the autouse env-isolation fixture strips the documented `CLAUDE_HOOKS_SOCKET_PATH` workaround from every wrapper subprocess. Remedy unbuilt |
