...
| N27 | a worktree daemon idles out in the middle of a full QA run                     | Plan 00461's agent                                                  | ⬜ Open — its hooks go to the main daemon, so after `idle_timeout_seconds: 600` the worktree daemon stops and `smoke_test` fails "Daemon not running"; agents and the coordinator's gate script now need a keep-alive                                                  |
