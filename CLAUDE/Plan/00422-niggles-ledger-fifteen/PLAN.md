...
| N18 | LSP.md relies on an `untracked/venv` symlink nothing creates             | chasing a Pyright diagnostic during issue #53                       | ⬜ Open — absent in the main checkout, created by no code, and bound to `LEGACY_VENV` by three scripts. Second doc-vs-code instance in one day (after N17). Remedy choice collides with the legacy meaning of the path                                                 |
| N19 | the Python nested-install check can never fire in a real client          | reviewing Plan 00455, whose agent copied it into `init.sh`          | ⬜ Open — its `pyproject.toml` exemption is true for every real client clone. The same shape nearly shipped in `init.sh`; review reverted it there                                                                                                                     |
| N20 | the acceptance probes cannot pass in a worktree whose daemon is running  | Plan 00456's final QA in its worktree                               | 🔄 Graduated to Plan 00458 — first diagnosis corrected: not a worktree fault. Six guards match skip lists as a bare substring, so a path segment ending in `venv`/`build`/`vendor` silently switches them off                                                          |
