...
| N22 | the local "full QA" never runs shellcheck                                | verifying Plan 00456's final QA before merge                        | ⬜ Open — `llm_qa.py all` (which `enforce_llm_qa` mandates) has no shellcheck tool; only `run_all.sh` and CI run it, so a shell defect surfaces only after merge                                                                                                       |
