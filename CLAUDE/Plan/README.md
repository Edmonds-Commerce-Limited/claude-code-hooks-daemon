...
- [00187: Socket discovery split-brain — CLI reconciliation](00187-socket-discovery-split-brain-cli-reconciliation/PLAN.md) - In Progress (field report `untracked/hooks-daemon-socket-mixup-upgrade.md`: after a v3.47→v3.48 upgrade, `status`/`health`/`restart` reported `NOT RUNNING` while hooks fired fine, because a stale git-tracked `hooks-daemon.env` pinned a non-canonical socket name (`-pda`) and `init.sh` consults the daemon's socket **discovery file** as a fallback but the Python management CLI does not. Fix: mirror `init.sh`'s discovery-file fallback in the CLI so `status`/`health` find the live daemon and emit an explicit split-brain drift warning instead of a bare NOT RUNNING; honour explicit `--socket`/`CLAUDE_HOOKS_SOCKET_PATH` verbatim)
...
- **Completed**: 150 (includes 1 reduced-scope plan and 4 found already-shipped when audited; count = `Completed/` folders)
