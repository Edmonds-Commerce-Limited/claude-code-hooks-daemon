...
- [00206: safe branch delete deterministic proof](00206-safe-branch-delete-deterministic-proof/PLAN.md) - In Progress (`destructive_git` blocks the force delete and the safe form refuses anything git calls unmerged, so a legitimately-stale branch has no deterministic route to deletion — only escalation to a human. Adds `hooks-daemon delete-branch` with a tiered, evidence-printing proof model.)
...
- [00174: Status-Line Artefact + Per-Segment Cadence Redesign](00174-status-line-artefact-cadence-redesign/PLAN.md) - In Progress (design brainstorm → looped audit/refine; NOT yet implementation-ready.)
...
- **Total Plans Created**: 206 (count = `hooksdaemon.latestPlanNumber` git counter; 00145, 00191 and 00195 were allocated by the counter but their folders are not present on this branch — 00195 was consumed by a transient probe during the v3.51.0 acceptance run)
...
- **Last reconciled by**: the Plan 00206 creation during the v3.52.0 release —
  counts re-derived from the folders on disk (`34` active, `163` completed),
  not carried forward. The prior reconciliation was the Plan 00205 creation at
  `33`/`163`; before that the Plan 00203 / 00204 creations at `32`/`163`.
