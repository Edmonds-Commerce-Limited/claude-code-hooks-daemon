...
- [00190: WorktreeCreate symlinks git-ignored env files](00190-worktree-create-seed-env-files/PLAN.md) - In Progress (usability: a fresh worktree checkout has none of the main working copy's git-ignored local env files — `.env.local`, `.env.test.local` — so a worktree agent runs against a config missing its local secrets/overrides. Makes `worktree_create` **symlink** a configurable `symlink_files` list from the repo top-level into the new worktree root on fresh creation only, keeping the main working copy as the single source of truth. Best-effort — a symlink failure logs and is skipped, never breaking the returned worktree path; never clobbers an existing destination; no re-seed on idempotent re-fire)
