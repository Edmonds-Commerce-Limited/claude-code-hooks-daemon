...
- [00191: WorktreeCreate symlinks git-ignored env files](00191-worktree-create-seed-env-files/PLAN.md) - In Progress (usability: a fresh worktree checkout has none of the main working copy's git-ignored local env files — `.env.local`, `.env.test.local` — so a worktree agent runs against a config missing its local secrets/overrides. Makes `worktree_create` **symlink** an opt-in `symlink_files` list from the repo top-level into the new worktree root on fresh creation only, keeping the main working copy as the single source of truth (relative links so they survive host↔container path remaps). **Fail-fast**: every configured entry must resolve to a file at the repo root or worktree creation aborts loudly; never clobbers an existing destination; no re-seed on idempotent re-fire. PR #35. Renumbered from 00190 after a merge-time collision with the plan-journal-separation plan)
...
- [00190: PLAN-vs-JOURNAL Separation & Tiered Plan Size Enforcement](00190-plan-journal-separation-and-size-enforcement/PLAN.md) - In Progress (research fan-out dispatched; design not yet fixed)
...
- **Total Plans Created**: 190 (count = `hooksdaemon.latestPlanNumber` git counter; 00145 was allocated by the counter but its folder is not present on this branch)
- **Completed**: 152 (includes 1 reduced-scope plan and 4 found already-shipped when audited; count = `Completed/` folders)
