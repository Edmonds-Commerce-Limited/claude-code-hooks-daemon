...
- [00228: prose guard for text-matching handlers](00228-prose-guard-for-text-matching-handlers/PLAN.md) - Not Started (one defect has recurred four times — a handler matching text that NAMES its trigger vocabulary rather than a command doing the thing; builds the class guard first, per Core Standard 15, then fixes the fourth instance as its failing test)
...
- **Total Plans Created**: 228 (count = `hooksdaemon.latestPlanNumber` git counter)
...
- **Folder-to-number reconciliation**: 32 + 184 + 6 = **222 folders**, spanning
  **219 distinct plan numbers** — three numbers carry two folders each, the
  historic collisions already held in `collision_allowlist` (00034, 00039,
  00041). Plans 1–3 are on disk under the pre-zero-padding names
  (`001-`, `002-`, `003-`), so they count as present. That leaves **9** of the
  228 allocated numbers with no folder: 00005, 00015, 00036, 00073, 00074,
  ...
  already covered the work). 219 + 9 = 228. ✅
- **Last reconciled by**: the Plan 00228 opening — one new root folder and the
  counter advanced by `mkplan.bash`, so Total and Active each rose by one while
  Completed and Cancelled were untouched (32 root, 184 `Completed/`,
