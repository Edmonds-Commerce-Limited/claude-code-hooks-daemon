...
- [00430: block report attributes before project context init](00430-block-report-attributes-before-project-context-init/PLAN.md) - In Progress (from issue #48: `block-report` never initialises `ProjectContext`, so five handlers fail construction during rule discovery and their denies are counted unattributed — a report that understates the guards whose rules it cannot see)
...
- **Total Plans Created**: 430 (count = `hooksdaemon.latestPlanNumber` git counter)

- **Completed**: 377 (includes 1 reduced-scope plan and 6 found already-shipped when audited; count = `Completed/` folders)
...
- **Folder-to-number reconciliation**: 30 + 377 + 13 = **420 folders**, spanning
  **415 distinct plan numbers** — three numbers carry two folders each, the
  historic collisions already held in `collision_allowlist` (00034, 00039,
  00041). Plans 1–3 are on disk under the pre-zero-padding names
  (`001-`, `002-`, `003-`), so they count as present. That leaves **13** of the
  428 allocated numbers with no folder: 00005, 00015, 00036, 00073, 00074,
