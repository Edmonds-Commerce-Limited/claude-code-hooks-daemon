...
- [00196: `absolute_path` acceptance tests assert an unreachable premise](00196-absolute-path-acceptance-tests-stale-premise/PLAN.md) - Not Started

  - Raised by the v3.51.0 acceptance gate under RELEASING.md "Never Drop Findings"; the gate is BLOCKING, so a false failure in it costs every future release run a re-diagnosis
  - Two `absolute_path` acceptance entries expect a relative `file_path` to be denied, but Claude Code normalises `file_path` to absolute BEFORE dispatching PreToolUse, so the daemon never sees the relative form
  - The **handler is correct** and stays: a socket probe of `Read` and `Write` with a relative path returns `deny` with the right message. Only the test premise is stale
  - Proven decisively: relative `untracked/relpath_probe.py` produced a `qa_suppression` block reading `File: /workspace/untracked/relpath_probe.py`
...
- **Total Plans Created**: 196 (count = `hooksdaemon.latestPlanNumber` git counter; 00145, 00191 and 00195 were allocated by the counter but their folders are not present on this branch — 00195 was consumed by a transient probe during the v3.51.0 acceptance run)
- **Completed**: 157 (includes 1 reduced-scope plan and 5 found already-shipped when audited; count = `Completed/` folders)
