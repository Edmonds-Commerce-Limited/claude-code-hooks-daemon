...
- [00185: Installer/Upgrade settings.json SSoT reconciliation + plan-workflow provisioning](00185-installer-upgrade-settings-ssot-and-plan-provisioning/PLAN.md) - In Progress (root-caused the session-start "Missing hook registration for {Event}" flood on installed/upgraded projects: settings.json hook reconciliation is neither SSoT-derived on every path nor merge-based, and `hook_registration_checker` only *detects* — never *adds* — missing wired events, so it nags forever. Deliverables: a shared `wired_event_metas()`-derived `reconcile_settings_hooks()` merge core; **session-time self-heal** in the checker (opt-out `auto_repair_registrations`) that stops the flood on an already-installed project WITHOUT a reinstall; removal of the drifting 15-event bash `generate_settings_json` fallback + an SSoT drift regression test; and Half B — an on-demand `deploy-plan-workflow` CLI + SessionStart drift advisory for the plan/journal assets (`mkplan.bash`/`_JOURNAL_TEMPLATE_.md`/`PlanJournalling.md`) that today are only ever deployed by a full install/upgrade. Shares the reconciler core with Plan 00176 (upgrade-time merge, which owns the shell wiring))
...
- **Completed**: 148 (includes 1 reduced-scope plan and 4 found already-shipped when audited; count = `Completed/` folders)
