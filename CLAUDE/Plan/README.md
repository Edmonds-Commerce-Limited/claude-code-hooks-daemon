...
- [00217: supervisor deployed into client owned path](00217-supervisor-deployed-into-client-owned-path/PLAN.md) - In Progress (field report: `claude-supervise.py` deploys into `.claude/ccy/`, which clients own and lint, rather than the `.claude/hooks-daemon/` vendor path they exclude. The three ruff findings themselves are deliberate and correct. Measured: they need `BLE`/`DTZ` selected — ruff's own defaults are clean — and the supervisor is one of FOUR daemon-owned assets deployed outside the vendor dir, none of which declared its ownership. Fixed by declaring the boundary in every asset and in one manifest, plus a guard that each stays clean under default rules.)
