...
- [00427: journal timestamps agent authored and timezone naive](00427-journal-timestamps-agent-authored-and-timezone-naive/PLAN.md) - Not Started, BLOCKED ON THE OWNER (from issue #45: the future-dated check reads a naive clock in the DAEMON process, so one clock judges every writer and only the future direction is judged — measured, not the mechanism reported; 2207 zoneless entries make the canonical-clock choice an owner call)
...
- **Total Plans Created**: 427 (count = `hooksdaemon.latestPlanNumber` git counter)
...
- **Folder-to-number reconciliation**: 30 + 374 + 13 = **417 folders**, spanning
