...
- [00190: WorktreeCreate seeds git-ignored env files](00190-worktree-create-seed-env-files/PLAN.md) - In Progress (usability: a fresh worktree checkout has none of the main working copy's git-ignored local env files — `.env.local`, `.env.test.local` — so a worktree agent runs against a config missing its local secrets/overrides. Makes `worktree_create` copy a configurable `copy_files` list from the repo top-level into the new worktree root on fresh creation only. Best-effort — a copy failure logs and is skipped, never breaking the returned worktree path; no re-copy on idempotent re-fire)
