...
- [00199: planlib — plan-orchestrator tooling in the daemon](00199-hooks-daemon-plan-lib/PLAN.md) - Not Started (implements the upstream proposal at `untracked/hooks-daemon-plan-lib-v2.md`, which supersedes the v1 document and renumbers its sections from §3.5 onward. The daemon owns the plan *lifecycle* — scaffolding, numbering, `PLAN.md` lint, commit gating, journals — but nothing about what happens when a plan needs something **run**. That half is hand-rolled per plan from a ~40-line safety preamble that diverges on every copy. Originating incident: a `triage.bash` resolved its repo root with `git rev-parse --show-toplevel`, which answers about the *cwd* not the script, so run from another checkout it operated on the wrong repo, checksummed a missing file and **exited zero** — the failure class being "a control that reports success without having done its job")

  - Ships proposal artefacts **1 + 3**: `_planlib.inc.bash`, a sourced bash library of safety-critical primitives (script-relative boundary-bounded root resolution; a run log with a *waitable* drain so the dying-process tail is never lost; `wait`-before-scrub ordering; `.unscrubbed` quarantine; `/dev/tty` prompts that do not race the log; the change gate plus a backstop assertion; the `BASH_SUBSHELL` guard), plus `test-planlib.bash` with a negative control on every control
  - **Defers artefact 2** (`plan_script_qa`): this repo has zero orchestrator scripts, so it would ship as a gate with an empty enforcement surface, an unseedable allowlist, and 16 rules validated only against the document that specified them. Deferral is recorded with explicit reopen triggers. When it is built, start with the seven mechanically crisp rules (R1, R2, R3, R5, R7b, R9, R12) — R14 (adversarial human review) is not checkable at all and belongs in docs
  - Two grounding finds: the codebase **already anticipated this file** — `config/models.py:544` and `plan_qa/model.py:431` both name `_planlib.bash` as the motivating example for `extra_root_files` — and the deployment vehicle already exists in `_deploy_mkplan` (`install/plan_workflow.py:277-292`), so the library ships daemon-owned at mode `0644` (sourced, not executed)
  - Dissent recorded: when artefact 2 is built its rules belong in the **existing** `plan_qa` catalogue, not a second parallel engine as the proposal's §6.1 specifies — that section was written against a project without `plan_qa`. Full assessment, rule triage, identifier-hygiene review and the v2 delta table in `PROPOSAL-ASSESSMENT.md`
  - v2 also supplies two verification techniques the plan adopts: `bash -n` **exits 0 while printing diagnostics**, so a syntax gate must assert stderr is empty (otherwise it is itself "a control that passes without checking"); and a perturbation probe must assert *exactly the expected assertion* failed — the author's first attempt injected `set -euo pipefail`, aborting the harness with zero assertions reported, which looked red and proved nothing

- [00200: QA Gate Integrity and Dogfooding False Positives](00200-qa-gate-integrity-and-dogfooding-false-positives/PLAN.md) - In Progress (two dogfooding defect classes surfaced by the presentation-quality audit. **The lint gate reports PASSED while checking nothing**: `untracked/qa/lint.json` records `total_files_checked: 0, total_violations: 0, passed: true` while ruff over the identical scope reports 47 violations. Root cause is a three-part composition — `venv-include.bash:81` writes `ensure_venv`'s success banner to **stdout** (its own error branches at `:91-95` correctly use `>&2`), `venv_tool():163` calls `ensure_venv` on every invocation, and `run_lint.sh:35` redirects stdout into the file it parses as JSON — after which the `except json.JSONDecodeError → ruff_output = []` swallow at `:55-57` converts the parse failure into a pass. This defeats a gate CLAUDE.md and RELEASING.md both call NON-NEGOTIABLE, and is itself the error-hiding pattern the repo ships a blocking handler and a dedicated auditor to prevent)

  - Verified the blast radius rather than assuming it: security/type_check/dependencies/format are all **honest** (their tools write JSON to a file via `-o`/`--json-output`, so `2>&1` cannot corrupt them; bandit and mypy ground truth are genuinely clean). Only ruff streams JSON to stdout, so only lint is corrupted — but that distinction is accidental, and the same swallow sits latent in `run_dependency_check.sh:74`, `run_security_check.sh:56`, `run_smoke_test.sh:150`
  - Second class: four handler false positives hit during ordinary work — `EnforceLlmQaHandler` blocking `cat` of `run_all.sh` (matches the filename, not the invocation); `destructive_git` reporting `git tag -f` as a force-push; `pipe_blocker` parsing a multi-line commit message body as shell; `lsp_enforcement` redirecting a `grep` already scoped to one named file. Each costs a turn and teaches agents to distrust the guardrail
...
- [00198: Installer Self-Destroying Symlink Fix](Completed/00198-installer-self-destroying-symlink-fix/PLAN.md) - Complete (presentation-quality audit found two git-tracked symlinks with absolute, author-local `/workspace` targets, one self-referential and dangling right now. Root cause: the deprecated `install.py`'s self-install deploy helpers had no `source == dest` guard, so `copy_slash_commands()` unlinked the real `.claude/commands/hooks-daemon-update.md` and symlinked it to itself, and `copy_init_script()` stored an absolute target. Deleted the dead symlink + deploy helper (superseded by the packaged upgrade skill, not resurrected); fixed `copy_init_script()` to emit a repo-relative target. Found and fixed the SAME hazard live in the current, non-deprecated `scripts/install/slash_commands.sh` — unreachable today only because both real orchestrators refuse self-install mode entirely, but tested/documented library code, so closed rather than left as a landmine. New `tests/integration/test_repo_symlink_hygiene.py` walks every git-tracked symlink (via `git ls-files -s`/`cat-file`, never the working-copy symlink) and fails on an absolute target, a dangling target, or a self-loop; confirmed RED against both live bugs before the fix. Client-mode verified via `scripts/dummy-client-repo.sh`: clean install, daemon RUNNING, `.claude/commands/` correctly absent. QA 14/14, 10838 tests. Delivery `9afd566d` + closure commit)

- [00197: Journal Day-File Today-Only Guard](Completed/00197-journal-dayfile-today-only-guard/PLAN.md) - Complete (cheap journal hygiene, automated: a Write/Edit targeting a plan `JOURNAL/NNNNN-Journal-YY-MM-DD.md` day-file dated anything other than TODAY -- including yesterday -- is now BLOCKED, closing the exact agent-mis-filing confusion a session spanning midnight used to trigger. New single-purpose check `journal-dayfile-is-today` (Stage.EDIT, block by default) owns write-time recency exclusively; the existing `journal-dayfile-naming` check is narrowed to grammar/plan-number/calendar-validity only, dropping its former today-or-yesterday tolerance, so the two checks can never give contradictory advice about one file. New independent config knob `plan_workflow.qa.journal.today_only_mode` (advise | block | off, default block) -- unlike the rest of journalling this does not ship advise-first, per explicit user request to enforce it immediately. Resident `plan_qa_edit` CLAUDE.md guidance updated; two new acceptance tests (block stale, allow today). Live-dogfooded against the restarted daemon via the real `pre-tool-use` hook script: a yesterday-dated Write was denied naming the exact today-dated filename to use instead, and an Edit appending to today's day-file passed silently. QA 14/14, 10830 tests, 95.3% cov. Delivery `e5ce3692` + closure commit)
