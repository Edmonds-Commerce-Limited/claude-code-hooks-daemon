...
- [00181: disk usage time bomb audit](00181-disk-usage-time-bomb-audit/PLAN.md) - Not Started (a read-only audit — three parallel review agents + on-disk measurement — confirmed a systemic pattern: the daemon writes many files under `untracked/` but **only one directory has a functioning reaper** (`temp/hooks/`, reaped each SessionEnd). Every other accumulator grows unbounded with no rotation/retention/pruning: stale venvs (~187 MB each, one per Python-fingerprint/slug, pruned only on `upgrade` or manual `prune-venvs`), `transcripts/` (one full-transcript snapshot per compaction, `transcript_archiver.py:91`, no cap), the append-only JSONL/log writers (`subagent_completions.jsonl`, `notifications.jsonl`, `stop-events.jsonl`, `hook-errors.log`), the supervisor `decision.log` (2.9 MB in one 6.5h session), per-session `thread-registry/`/`context-sidecar/`/`payload-capture/` files (no recursive sweep), and 44 orphaned `daemon-*` runtime files (reaper EXISTS at `paths.py:1439` but is BYPASSED by the Plan 00127 reuse gate returning before the sweep at `cli.py:411`, plus a 7-day age floor). Decisive: a grep for `RotatingFileHandler`/`maxBytes`/`rotate` across the codebase returns nothing — there is no retention primitive at all. Plan introduces ONE shared config-driven retention utility, applies it to every unbounded writer, closes the reaper-invocation gaps, makes stale-venv pruning automatic (never deletes current fingerprint), and adds a `disk-usage` diagnostic CLI. Findings + file:line table recorded in PLAN.md; implementation not started)

- [00182: supervisor compact stacking / double-inject](00182-supervisor-compact-stacking-double-inject/PLAN.md) - Not Started (live dogfooding bug: the ccy PTY supervisor injected TWO `/compact` commands back-to-back — the second ran after the first had already compacted, producing "Not enough messages to compact." Code-VERIFIED root cause is NOT a state-machine defect (`AWAIT_COMPACTING` correctly prevents in-process re-injection) but a **host ↔ policy-worker IPC desync**: `PolicyWorker.decide()` (`claude-supervise.py:2362-2385`) is a one-write/one-blocking-read protocol with **no correlation id and no stale-reply drain**, and its read timeout `_WORKER_READ_TIMEOUT_SECONDS=2.0` (line 125) **equals** the poll interval `_DEFAULT_POLL_SECONDS=2.0` (line 1822). When a worker tick exceeds 2s (it scans all of `/proc` via `cached_own_session_ids`) the host times out (`select` not-ready, 2374-2375), falls back in-process and injects `/compact` #1; on the NEXT tick `readline()` (2377) returns the worker's buffered STALE `WOULD_COMPACT` reply and `_apply_decision` (2101) injects `/compact` #2 because it injects any non-None payload WITHOUT re-checking the host's authoritative `AWAIT_COMPACTING` state. Plan 00164 fixed the fallback-machine divergence but not this pipe-buffer divergence; supervisor never parses child output (timing only, 2552) so the no-op is invisible. Unrelated to Plan 00180 (cap never approached). Fixes: (1) correlate/drain worker replies by tick id; (2) state-guard `_apply_decision` against injecting while `AWAIT_COMPACTING`; (3) cache the `/proc` scan and/or decouple read-timeout from poll-interval. Verified root cause + reproduction path in PLAN.md; TDD implementation not started)
...
- **Total Plans Created**: 182 (count = `hooksdaemon.latestPlanNumber` git counter; 00145 was allocated by the counter but its folder is not present on this branch)
- **Completed**: 144 (includes 1 reduced-scope plan and 4 found already-shipped when audited; count = `Completed/` folders)
