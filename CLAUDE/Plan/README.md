...
- [00446: subagent report path claim is verified](00446-subagent-report-path-claim-is-verified/PLAN.md) - Not Started (from 00422 N15: an agent claimed `Report written to:` a path that was never created, and no caller-stated value can catch a false EXISTENCE claim — so the check runs where the claim and the filesystem are both in reach)
...
- **Total Plans Created**: 446 (count = `hooksdaemon.latestPlanNumber` git counter)

- **Completed**: 393 (includes 1 reduced-scope plan and 6 found already-shipped when audited; count = `Completed/` folders)
...
- **Folder-to-number reconciliation**: 30 + 393 + 13 = **436 folders**, spanning
  **433 distinct plan numbers** — three numbers carry two folders each, the
  historic collisions already held in `collision_allowlist` (00034, 00039,
  00041). Plans 1–3 are on disk under the pre-zero-padding names
  (`001-`, `002-`, `003-`), so they count as present. That leaves **13** of the
  446 allocated numbers with no folder: 00005, 00015, 00036, 00073, 00074,
