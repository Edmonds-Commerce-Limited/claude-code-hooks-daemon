...
- [00279: generic agent install subsystem](00279-generic-agent-install-subsystem/PLAN.md) - In Progress (first-class daemon-shipped agent deployment: version + md5 ledger, customisation detection that never clobbers, config-gated deploy/remove lifecycle, `hooks-daemon agents` CLI; first payloads: plan-dedupe scout migration + the opus-security quarantine agent)
...
- **Total Plans Created**: 279 (count = `hooksdaemon.latestPlanNumber` git counter)

- **Completed**: 229 (includes 1 reduced-scope plan and 5 found already-shipped when audited; count = `Completed/` folders)
...
- **Folder-to-number reconciliation**: 40 + 225 + 6 = **271 folders**, spanning
  **268 distinct plan numbers** — three numbers carry two folders each, the
