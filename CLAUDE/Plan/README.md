...
- [00183: Supervisor dry-run fires once per session](00183-supervisor-dry-run-fire-once/PLAN.md) - In Progress (live dogfooding bug: in **dry-run** mode the ccy PTY supervisor loops, re-emitting "would send [esc]" / "compact suggestion fired" markers on every tick and flooding the session with fake prompts that each wake the agent. Root cause: the state machine is pure and identical in dry-run and armed modes — armed mode's real `/compact` changes the world so each episode resolves, but a dry-run marker is a no-op on the environment (and is Enter-submitted as a real prompt), so context stays red and the machine re-decides `MONITOR → WOULD_COMPACT → AWAIT → WOULD_ESCAPE×N → …` up to `max_injections`. Fix per the user's spec — *dry-run fires once per session, once only*: a process-lifetime `_dry_run_fired` latch on `CompactStateMachine` (carried in `export_state`/`import_state` so it survives the Plan 00164 policy-worker round-trip), gated at the single choke point `decide_once` — the first dry-run marker fires, every one after is suppressed to a logged NOOP. Armed mode untouched)

- [00184: venv accounting is symlink-aware and protects the live venv](00184-venv-accounting-symlink-aware/PLAN.md) - In Progress (live dogfooding data-loss incident found while reclaiming the "170.8 MB reclaimable legacy venv" that `disk-usage`/`list-venvs` reported: `prune-venvs --legacy --force` deleted the venv the running daemon + hook forwarders were actually using, because `untracked/venv-py311-66bbc57c` was a **symlink** to the real `untracked/venv`. Two defects in `daemon/cli.py`: (1) `_enumerate_venvs` used `is_dir()`/`rglob` through the symlink so a symlink + its target were counted as two independent venvs with the same bytes counted twice; (2) protection keyed on `python_venv_fingerprint` (the slug management scheme) but the daemon/forwarders run the venv chosen by the bootstrap resolver `resolve_existing_venv_python` — when the two schemes disagree (fingerprint migration + leftover symlink) the live venv was flagged reclaimable and pruned. Fix: new `_resolver_active_venv_realpath` helper; `_enumerate_venvs` dedupes by realpath and marks `is_current` as the UNION of the fingerprint match and the resolver-active realpath; `_reclaimable_venv_entries` skips symlinks; `cmd_prune_venvs` refuses to delete a symlink entry or the resolver-active venv. Recovered live by rebuilding the correct `venv-workspace-py311-81c29529`)
...
- **Total Plans Created**: 184 (count = `hooksdaemon.latestPlanNumber` git counter; 00145 was allocated by the counter but its folder is not present on this branch)
- **Completed**: 146 (includes 1 reduced-scope plan and 4 found already-shipped when audited; count = `Completed/` folders)
