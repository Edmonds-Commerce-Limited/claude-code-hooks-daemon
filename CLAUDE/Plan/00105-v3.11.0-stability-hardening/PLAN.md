...
- [ ] ⬜ **Task 2.1**: Write `scripts/qa/run_capture_corruption_check.sh` — for every `*.sh` under `scripts/install/`, parse function definitions, find any function whose last statement is `echo "$value"` (return-via-stdout pattern), and assert that no other statement in the function body emits to stdout. Any candidate `echo`/`printf` without `>&2` redirection is a violation.
- [ ] ⬜ **Task 2.2**: Wire as 13th QA gate in `scripts/qa/run_all.sh` and `scripts/qa/llm_qa.py`. Output JSON to `untracked/qa/capture_corruption.json`.
- [ ] ⬜ **Task 2.3**: Run the new check, fix any violations it finds (Phase 3 audit covers this — the check formalises Phase 3's audit as a permanent gate).
- [ ] ⬜ **Task 2.4**: Add a focused integration test that exercises every `scripts/install/*.sh` function called via `VAR=$(fn ...)` in any caller. For each, assert capture is uncorrupted.
