...
`reference_repo_freshness` says BLOCKED on a call it allows
...
the regenerated CLAUDE.md guidance block is not deterministic, so every daemon restart can commit a reorder
