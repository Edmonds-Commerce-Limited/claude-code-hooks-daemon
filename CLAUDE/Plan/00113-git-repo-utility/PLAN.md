...
- [ ] **Task 1.1**: Confirm the two config consumers + scope (done in review)
- [ ] **Task 1.2**: Author this PLAN.md

### Phase 2: TDD GitRepo

- [ ] **Task 2.1**: RED — `tests/unit/utils/test_git_repo.py`: resolve_for
...
- [ ] **Task 2.2**: GREEN — implement `utils/git_repo.py`
- [ ] **Task 2.3**: 100% coverage on the new module

### Phase 3: Migrate consumers

- [ ] **Task 3.1**: `plan_numbering` delegates to `GitRepo`; delete its `_git_output`
- [ ] **Task 3.2**: `git_filemode_checker._get_filemode_setting` uses `GitRepo.read_config`
- [ ] **Task 3.3**: Update `error_hiding_exclusions.json` (move `_git_output` entry to
  `utils/git_repo.py`; drop entries that no longer apply)
- [ ] **Task 3.4**: Keep all existing tests green (update mocks/patch targets)

### Phase 4: QA + close

- [ ] **Task 4.1**: `./scripts/qa/llm_qa.py all` — 13/13, coverage ≥ 95%
- [ ] **Task 4.2**: Restart daemon, verify RUNNING
- [ ] **Task 4.3**: Mark complete with commit hashes, archive to `Completed/`, update README

## Success Criteria

- [ ] `GitRepo` is the single home for resolve / read_config / write_config.
- [ ] `plan_numbering` and `git_filemode_checker` both delegate to it; neither
  hand-rolls a `git config` subprocess any more.
- [ ] No behavioural change — all existing plan-number and filemode tests pass.
- [ ] QA 13/13, coverage ≥ 95%, daemon restarts RUNNING.
