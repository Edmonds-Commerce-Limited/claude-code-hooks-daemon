...
| `strategies/tdd/common.py:26` (`matches_directory`)                                                            | `for directory in directories: pattern = f"/{directory}/"; if pattern in file_path`  | Yes (follow-up)         | Already both-side segment-bounded by construction; only the project-relativity half was missing, initially left as a documented follow-up. Fixed per owner instruction ("no half measures") — see the follow-up section below. |
...
Commit: `fae188fa`. Merged `main` first (`b8ce4b49` onward, clean, no
conflicts). Full QA re-run after this fix, daemon restarted:

```
✅ tests: 25668 passed, 0 failed, 24 skipped | coverage: 95.1%
QA: 36/36 PASSED
```
