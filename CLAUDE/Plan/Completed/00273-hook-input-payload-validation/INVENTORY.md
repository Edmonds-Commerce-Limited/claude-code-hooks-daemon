...
| PostToolUse       | `cwd`, `session_id`, `tool_input`, `tool_name`, `tool_response`, `transcript_path`                                                                                |
| PreCompact        | `session_id`, `transcript_path`                                                                                                                                   |
| PreToolUse        | `cwd`, `scratchpad_dir`, `session_id`, `tool_input`, `tool_name`, `transcript_path`                                                                               |
...
### v2.1.252 refresh — newly documented input fields

| Field                                                                                                                                                                                                 | Verdict                                                                                                                                                  |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `agent_type`, `session_title` (SessionStart)                                                                                                                                                          | Recorded gap — no SessionStart handler branches on agent name or existing title; consume if a title-setting or agent-scoped startup feature appears.     |
| `seconds_since_last_response`, `context_tokens`, `prompt_cache_likely_expired`, `estimated_cache_write_usd` (SessionStart, resume/fork only)                                                          | Recorded gap — resume-cost reporting is a plausible advisory feature but nothing consumes these today (Claude Code v2.1.251+).                           |
| All PreModelSwitch / PostModelSwitch input fields (`from_model`, `to_model`, `requested_model`, `source`, `context_tokens`, `prompt_cache_warm`, `cache_ttl`, `estimated_cache_write_usd`, `pricing`) | Recorded gap — the events themselves are catalogued `wired=False` (EXPECTED_UNWIRED); triage their fields for consumption in the change that wires them. |

### v2.1.272 refresh — newly documented input fields

| Field                         | Verdict                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `scratchpad_dir` (PreToolUse) | **Already consumed** — `handlers/pre_tool_use/project_containment.py` reads it to treat the per-session scratchpad as a permitted write target. It was consumed BEFORE upstream documented it, so it was carried as an `INPUT-ALLOWLIST.yaml` docs-gap entry; upstream now documents it as a common input field requiring Claude Code v2.1.257+, the vendored `PreToolUse.json` example carries it, and that allowlist entry was deleted in the same commit. The read fails closed — a payload without the field grants nothing. |
