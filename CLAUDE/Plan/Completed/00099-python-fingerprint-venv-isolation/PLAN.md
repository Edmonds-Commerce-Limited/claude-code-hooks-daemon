...
- [x] ✅ **Task 3.1**: `scripts/install/venv.sh` ships `create_venv_at_path` (explicit path) alongside the legacy `create_venv` (Phase 2 landed this)
  - [x] ✅ `ensure_venv` orchestrates fingerprint-keyed path resolution + creation + stamp
  - [x] ✅ Legacy `create_venv` / `recreate_venv` kept for backwards compatibility with external callers
- [x] ✅ **Task 3.2**: Migrate `scripts/install_version.sh` + `scripts/upgrade_version.sh` to `ensure_venv`
  - [x] ✅ Both source `scripts/install/python_fingerprint.sh`
  - [x] ✅ `VENV_PATH=$(ensure_venv "$DAEMON_DIR" "$TARGET_VERSION")` — fingerprint-keyed venv provisioning
  - [x] ✅ `VENV_PYTHON` derived from returned `VENV_PATH` (no more hardcoded `untracked/venv/`)
  - [x] ✅ Upgrade script: `_resolve_existing_venv_python()` helper for pre-upgrade probes (tries fingerprint-keyed, falls back to legacy)
  - [x] ✅ Both scripts: after successful fingerprint-keyed provisioning, remove legacy `untracked/venv/` ("don't leave legacy venvs lying around")
- [x] ✅ **Task 3.3**: `scripts/venv-include.bash` (sourced by all 8 QA scripts) uses `_resolve_venv_dir` with the same precedence chain as init.sh — auto-propagates fingerprint-keyed venv to run_all.sh, run_tests.sh, run_lint.sh, etc.
  - [x] ✅ 5 integration tests in `test_venv_include_resolution.py` (explicit override, fp-keyed over legacy, legacy fallback)
- [ ] ⬜ **Task 3.4**: Update `.claude/hooks-daemon.env` generation (deferred to Phase 5 — env file is regenerated by `install` skill, and the hostname-scoped SOCKET_PATH/PID_PATH are already correct)
...
- [ ] ⬜ **Task 4.2**: Call `ensure_venv` at init (guarded by CI gate) — **DEFERRED**
