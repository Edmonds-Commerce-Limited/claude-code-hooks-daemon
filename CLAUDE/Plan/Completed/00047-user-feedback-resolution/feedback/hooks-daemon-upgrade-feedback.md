...
07. **Deduplicate priorities in default config**: Assign unique priorities to avoid 18 warnings
08. **Add Python version requirement to LLM-UPDATE.md**
09. **Add socket path troubleshooting to LLM-UPDATE.md**
...
## Second Attempt: Fresh Install with FORCE=true (v2.10.0/main)

After the failed upgrade, we ran the install script with `FORCE=true` which deleted the existing `.claude/hooks-daemon/` and re-cloned from main branch. This triggered the Layer 2 installer (`install_version.sh`) which worked significantly better.

### What Layer 2 Install Did Correctly

- Auto-detected Python 3.13 (didn't use system python3 which is 3.9)
- Installed uv automatically
- Created venv with correct Python
- Deployed 11 hook forwarder scripts to `.claude/hooks/`
- Generated `.claude/settings.json` with ALL event types (StatusLine, SessionStart, PreCompact, Stop, SubagentStop, UserPromptSubmit, etc.)
- Generated `.claude/hooks-daemon.env`
- Generated `.claude/hooks-daemon.yaml`

### What Layer 2 Install Did NOT Handle

- **Socket path auto-fallback mismatch (Issue 10 below)**
- Exited with code 1 at Step 8 (.gitignore setup) - non-fatal but alarming exit code
- Did not start the daemon (it auto-starts lazily on first hook call, which is fine)

---

## Issue 10: Python Auto-Fallback for Socket Path Doesn't Match Bash init.sh

### What happened

The updated LLM-UPDATE.md (lines 790-793) says:
...
This is TRUE for the **Python daemon** (`daemon/paths.py`). When started without override, it correctly started on `/run/user/1000/hooks-daemon-aee977c2.sock`.

BUT the **bash init.sh** (sourced by all hook forwarder scripts) computes the socket path independently using pure bash and does NOT have this fallback logic. It always computes the default path:

```
$_untracked_dir/daemon${_hostname_suffix}.sock
```

which is too long.

Result: The Python daemon is running on `/run/user/1000/...` but the bash forwarders try to connect to the too-long path in `untracked/`. They can't find the daemon and report "daemon not running".

### Fix suggestion

Two options:

1. **Add matching fallback logic to init.sh** - compute path length, fall back to same directories as Python
2. **Have the daemon write its actual socket path to a discovery file** (e.g., `untracked/daemon.socket-path`) that init.sh reads instead of computing

Option 2 is simpler and eliminates the dual-computation problem entirely.

### Workaround

Set explicit override in `.claude/hooks-daemon.env`:
...
This forces both Python daemon and bash init.sh to use the same path. Using `/run/user/{uid}/` is better than `/tmp/` as it's user-scoped and matches the daemon's preferred fallback.

---

## Updated Docs Review (Second Fetch of LLM-UPDATE.md)

The latest version of LLM-UPDATE.md has fixed several issues from our first report:

| Issue                          | Status in Updated Docs                                                                    |
| ------------------------------ | ----------------------------------------------------------------------------------------- |
| Python 3.11+ requirement       | **FIXED** - Lines 54-67, new Prerequisites section                                        |
| Restart Claude Code            | **FIXED** - Line 50, now says "RESTART REQUIRED for multi-version upgrades"               |
| Socket path troubleshooting    | **FIXED** - Lines 784-798, new section with auto-fallback description                     |
| Broken install recovery        | **FIXED** - Lines 800-813, new section                                                    |
| Layer 1 checkout-first         | **FIXED** - Line 75 mentions "checkout-first strategy"                                    |
| `--project-root` required      | **FIXED** - Lines 111-112                                                                 |
| Feedback section               | **ADDED** - Lines 890-977                                                                 |
| init.sh / Python path mismatch | **NOT FIXED** - Auto-fallback only in Python, not bash (Issue 10)                         |
| settings.json during upgrade   | **PARTIALLY FIXED** - Layer 2 handles it, but manual upgrade steps still don't mention it |
