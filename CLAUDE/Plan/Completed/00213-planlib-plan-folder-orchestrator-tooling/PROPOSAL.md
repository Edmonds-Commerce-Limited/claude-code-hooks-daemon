...
**Status**: proposal for upstream, ready to hand over.

**Provenance of the code below**: it is running in a private infrastructure repository. This
document is a **generic extraction**, not a copy of that repo — every project-specific fact has
been pulled out into a configuration seam (§2), and the `_planlib.inc.bash` presented in §3 is
complete and self-contained rather than excerpted.

**Verification of this document** — stated up front, because a proposal containing code that has
never been run is a proposal that costs the reader the debugging:

| What                  | How                                                                       | Result                 |
| --------------------- | ------------------------------------------------------------------------- | ---------------------- |
| The library in §3     | blocks concatenated into one file, `bash -n` **plus assert stderr empty** | clean                  |
| The same file         | `shellcheck -x -S style`                                                  | clean                  |
| Every function called | cross-referenced against every function defined                           | no dangling references |
| The skeletons in §5   | **not** executable — they contain `<BOOTSTRAP>` and `<cmd…>` placeholders | marked as templates    |

`bash -n`'s exit code is not trusted on its own: for a malformed conditional it prints a
diagnostic to stderr and **still exits 0**, so silence on stderr is the actual signal.
...
A hand-copied preamble diverges on every copy. That is structural, not a discipline problem. The
dangerous primitives belong in one tested library where the correct behaviour is the **only**
behaviour on offer.

### 1.3 Why this belongs in the daemon

The daemon already owns the *plan lifecycle* — it scaffolds plan folders, tracks the plan number
in a git counter, lints `PLAN.md` at Write/Edit, gates plan invariants at `git commit`, and
advises on journals. It owns nothing about what happens when a plan needs something **run**,
...
| --- | ------------------- | ------------------------------------------------------ | --------------------- |
| 1   | `_planlib.inc.bash` | A sourced bash library of safety-critical primitives   | nothing but bash 4.2+ |
| 2   | `plan_script_qa`    | A handler enforcing that orchestrators are built on it | 1                     |
...
The library as it exists is Ansible-flavoured. That flavour is **one seam**, not a design
assumption. Nothing above the seam knows what a playbook is.
...
│  · PLANLIB_CHECK_FLAG       the dry-run flag             │
│  · PLANLIB_FORCE_COLOR_VAR  colour-forcing env var       │
└──────────────────────────────────────────────────────────┘
```

An Ansible project points `PLANLIB_DELEGATE` at its `ansible-run.bash` wrapper. A Kubernetes
project points it at a context wrapper. A project with no runner leaves it unset and legs call
commands directly — everything else still applies.

---

## 3. The generic core, complete

The blocks in this section concatenate, in order, into a working `_planlib.inc.bash`. Every
load-bearing decision carries its reason inline, because these are precisely the comments that
stopped being copied.

### 3.1 Header and the configuration seam
...
something is worse than nothing here — that is §1.1 exactly. It must also never be `.git`,
because `.git` is the **boundary** (§3.3), and if marker and boundary are the same file the
...
The `|| return 1` convention is not stylistic. Under `set -e` a bare `_plan_err "..."` returns
non-zero and terminates the *caller* at that line — so the `return 1` you wrote after it never
runs, and any cleanup between them is skipped. One expression makes control flow identical
whether or not errexit is on.

### 3.3 Root resolution — the primitive §1.1 was about
...
Three properties, each got wrong in the wild:

1. **Script-relative, not cwd-relative.** The walk starts at the calling script's own directory.
   `git rev-parse --show-toplevel` answers a different question, confidently.
2. **Filesystem-only.** No `git` invocation. Worktree git links do not resolve identically across
   versions and mount layouts, and a subprocess is a dependency the walk does not need.
3. **Bounded at the repository boundary.** Nested checkouts are ordinary — vendored repos, a
   template checked out inside its consumer. An *unbounded* walk from a script in the inner repo
   sails past it, finds the **outer** repo's marker, and appears to work. Failing loudly beats
...
### 3.5 ssh-agent loading — ordering as an enforced contract
...
- **Distinguish "agent up but empty" (1) from "no agent" (2) from "ssh-add missing" (127).** Every
  failure is *reported*; the listing is empty when unknown, so the caller falls through to a load
  attempt rather than wrongly skipping a key.

### 3.6 The run log — the part most likely to be got wrong

Three separate subtleties here, each producing a **silently truncated forensic record**.
...
substitution **cannot be waited on**. There is no PID to `wait`, so the final buffered chunk may
never reach the file — and the final chunk is the lines written as the run was dying, which is
the reason you kept a log at all.
...
stages separately gives a PID whose completion says nothing about the stripper.

**Subtlety 3 — arm for signals, not just EXIT.** The interrupt case is precisely the one where
the tail of the log carries the diagnosis.
...
now reports itself clean. That is §1.1's failure class again: a control that ran, reported
success, and left the thing it was checking untouched.

### 3.7 Log scrubbing — layered, and the layers are ordered
...
Layer 2 must never become the reason a task omits layer 1 — that inverts the ordering and makes
the last line of defence the first.

**A clean scrub means "none of the KNOWN shapes are present"**, never "this log is safe".
Whatever the scrubber cannot reach belongs in the scrubber's own header, not implied.

**Why `.unscrubbed` rather than deleting or failing**: the run already happened and its output is
the forensic record. Deleting destroys evidence; failing after the fact does not un-write it.
Renaming into a gitignored name means the *only* thing you can lose is the log's presence in git,
and never an uncleaned log's presence in a commit.

### 3.8 Prompts, and why they must not bypass the log
...
Four independent traps, each with a visible symptom:

| Trap                                | Symptom if got wrong                                                          |
| ----------------------------------- | ----------------------------------------------------------------------------- |
| Prompt written to `/dev/tty`        | prompt appears **above** its own banner — unbuffered, races the tee           |
| Prompt without a trailing newline   | run **wedges with a blank screen** — partial line block-buffers               |
...
### 3.9 The change gate — gate on state change, never on target name
...
**The conventional model is "gate production, wave the rest through".** That assumes most
environments are safe to break. Where there is one environment and it is live, name-gating gates
...
`_plan_assert_change_allowed` is what makes the gate more than a convention: forgetting it is an
...
**The rule: a plan script never rebuilds what the project's runner already does.**
...
bastion `ProxyCommand`, and shreds every credential on the way out via an EXIT trap. A plan script
that hand-rolls the two obvious flags gets those right and **silently drops all the rest** —
producing a script that looks correct and cannot reach a single host.

Generically: whatever your runner does beyond "assemble two flags" is invisible to the person
copying the two flags.
...
the subshell — so a *failed* leg would print `[ABORT]` and the script would carry on to the next
one. The run looks aborted and is not. `kill -TERM "$$"` takes the real process down.

There is deliberately **no** "continue past a failed deploy leg to gather more". If you want
that, the run is a gather.

### 3.12 Closing and the flag vocabulary
...
**`plan_finish` exists so the exit status agrees with the text.** A script that prints a summary
and falls off the end returns the status of whatever ran last, which is routinely `0` after a
`printf`. A run with failed gather legs exiting `0` is §1.1's failure class again.

### 3.13 The one deliberate deviation from "a library never exits"

Standard guidance says a sourced library must never call `exit`, because it runs in the caller's
shell and would kill the caller mid-script, skipping cleanup traps.

Three functions break that on purpose: `plan_deploy_leg`, `plan_finish`, and
`plan_parse_common_flags --help`. For the first two **"abort the whole run" is the contract**, and
delegating it to the caller (`|| exit 1` at every call site) reintroduces exactly the catastrophe
the primitive exists to prevent: one forgotten guard and a failed leg flows into the next.

Two things keep it honest:

- The library sets **no** shell options. The caller owns `set -euo pipefail`.
- **The suite pins the deviation to exactly those three functions**, so a fourth cannot grow one
  unnoticed. Extending the exception means extending that test.
...
`source-path=SCRIPTDIR` makes the relative `source=` path resolve from the *script's* directory
rather than shellcheck's cwd. Without it `shellcheck -x` cannot follow the library, emits SC1091,
and **every check that depended on following it silently lapses** (`PLAN_USAGE` starts reading as
an unused variable, and so on).

The subtle part: **a plan folder moves.** While active it sits at `<plan_dir>/NNNNN-name/`, where
`..` is `<plan_dir>/`. Completing the plan `git mv`s it to `<plan_dir>/Completed/NNNNN-name/`,
where `..` is `<plan_dir>/Completed/` — the library is no longer there, SC1091 fires, and the
cascade returns.

If CI runs a bare `shellcheck -x` with **no severity floor**, an *info*-level SC1091 fails the
build. So **archiving a plan turns `main` red**, and the commit doing it looks entirely unrelated
to shell code. Listing both `SCRIPTDIR/..` and `SCRIPTDIR/../..` makes the directive
...
**These are templates, not runnable files** — `<BOOTSTRAP>`, `<MARKER>` and `<cmd…>` are
placeholders. Each is **bootstrap + mode + legs**; the library carries everything else.
...
## 6. The QA handler (`plan_script_qa`)

### 6.1 The two-file split, and why it matters for CI
...
part that has actually had bugs.
...
### 6.2 Orchestrator versus helper
...
Wrong in either direction is costly: too broad and the gate demands a change gate from a helper
that must not have one; too narrow and a real orchestrator escapes every structural rule.

### 6.3 The rules

**A deliberate deviation carries `# STANDARD-EXCEPTION(Rn): reason` on the offending line**, which
downgrades that rule to advisory *and echoes the reason*. It is deliberately **not** a
`# shellcheck disable`-style suppression: it is loud, and the reason is the point.
...
| R6  | Delegate to the project runner                                                   | a hand-rolled runner argv carrying credentials                                                  |
| R7  | Declare mode; use the matching leg runner                                        | `plan_deploy_leg` in gather mode, or vice versa                                                 |
| R7b | `plan_deploy_leg` is a bare top-level statement                                  | it inside `$( )`, `( )` or a pipeline stage — see below                                         |
...
| R12 | Ship it executable                                                               | most agent file-writers create `0644`; `./script.bash` then fails                               |
...
**R7b is new in this proposal, and it exists because the runtime guard is not enough.**
`BASH_SUBSHELL` catches the misuse only when that line executes — so a `plan_deploy_leg` buried in
a rarely-taken branch ships undetected. It is statically detectable, and a static rule costs
nothing:

```python
# A deploy leg inside a command substitution, an explicit subshell, or any pipeline stage.
# Its `exit` would end only the subshell, so a FAILED leg prints [ABORT] and the run
# continues to the next one — aborted in appearance, running in fact.
_RE_LEG_IN_SUBSHELL = re.compile(
    r"""(?x)
...
The operator's first action is `./deploy.bash`, which fails with `Permission denied`. It is worth
a gate purely because it is worth never explaining again.
...
core code by definition. Promoting it out is part of *finishing* the plan.

### 6.4 A representative rule implementation

R15, which must distinguish "runs project code" from "carries its own":
...
def _target_is_core(path: str) -> bool:
    ...
    read as core: a gate that guessed 'plan-local' from an expansion it cannot resolve
...
Four principles it embodies, all of which generalise:
...
1. **Keep predicates pure so they are testable without the world.** `_plan_fingerprint_present`
   ...
   testable with no agent running. Same for `_plan_find_repo_root` and `_plan_strip_cr`.
...
3. **Every control gets a negative control, and perturbation is how you prove it.** A gate that
   fires is not necessarily a gate that *discriminates*. Perturb **one** thing and require exactly
   the expected assertion to fail — a uniform failure across unrelated assertions proves nothing
   about any of them. Worked example from validating this proposal: removing the `HUP` line from
   `plan_start_log`'s trap block produced exactly one failure, naming HUP. Injecting
   `set -o pipefail` into the library produced exactly one failure, naming the shell-options rule.

   The first attempt at that perturbation used `set -euo pipefail`, which aborted the harness on
   its first assertion — rc=1, zero assertions reported. **A red suite is not the same as the
   right assertion firing**, and a probe that cannot tell them apart proves nothing.

4. **State what the suite cannot cover.** A TTY cannot be faked from a container, so the suite
   covers the no-TTY paths via `setsid` and the library header carries a **manual** checklist:
...
plan_workflow:
  scripts:
    # Required. No default on purpose — see §3.1.
    ...
    # Optional: env var set to 1 when the console is a TTY.
    force_color_var: "ANSIBLE_FORCE_COLOR"

    # Optional: the secret scrubber, repo-relative.
    scrubber: "shellscripts/scrub-secrets.py"

    # When true the scrubber is REQUIRED and a failed scrub quarantines to `.unscrubbed`.
    track_run_logs: true
...
**`warn` as the rollout default is deliberate.** A gate that starts red against every existing
file is one everybody learns to skip.

---

## 9. What is deliberately NOT proposed

- **A plan-script *generator*.** Scaffolding the orchestrator would produce files that exist
  because a tool made them. The skeletons in §5 are short enough to copy deliberately.
...
- **Anything that runs plan scripts automatically.** These are operator-invoked by design. The
  ...
  as an isolation guarantee would be false, and false in the direction that invites people to rely
  on isolation nobody is maintaining.
...
Stated because an unstated limit reads as coverage. Each of these is a limit rather than a defect
— anything on this list that *could* be closed has been (R7b in §6.3 was on this list in an
earlier draft and is now a rule).

- **Bash 4.2+.** `BASH_SUBSHELL`, `${arr[@]+"${arr[@]}"}` and `mkfifo` semantics are assumed. No
...
- **`plan_run` cannot verify that your delegate is safe.** It guarantees only that the plan script
  did not re-derive it. If the delegate leaks credentials, R6 has not helped you.

- **`is_orchestrator` is heuristic** — basename plus a `plan_mode` declaration. A script named
  something else that declares no mode escapes the structural rules. That is the intended trade,
  but it is a trade.

- **`plan_list_reports` matches `*report*` by filename.** A report named something else is not
  listed. Deliberate: the alternative is a manifest the tool must be told to write.
...
2. **`plan_script_qa` in `warn`**, seeded with a `legacy_script_allowlist` of what exists, so it
   ships green.
3. **Flip to `block`** once the allowlist is empty. Make the allowlist a **ratchet**: a stale entry
   *fails* rather than being a no-op, and growth fails — otherwise it stops being a ratchet and
   becomes a list of names nobody can justify.

The ordering matters for the same reason the gate defaults to `warn`: the goal is a control people
keep, and a control that arrives red is a control that gets switched off.
