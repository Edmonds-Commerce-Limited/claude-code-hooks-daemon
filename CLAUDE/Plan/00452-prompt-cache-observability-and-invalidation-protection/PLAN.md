...
- [ ] ⬜ **Task 4.4**: Advisory on edits to prefix-resident files, quoting the
  ...
  the prefix is smallest. **Blocked on evidence, and this matters**: the
  measured misses are 13 `messages_rewritten` and 2 `effort_changed`, and NONE
  is attributed to a prefix-file edit. This project's `CLAUDE.md` was
  regenerated by seven daemon restarts inside the measurement window without
  moving the miss count. So the premise that editing a prefix-resident file
  invalidates the cache is currently UNSUPPORTED by our own data. Shipping the
  advisory anyway would violate the success criterion that every advisory rule
  trace to a Phase 3 observation rather than to a document. What would settle
  it: a deliberate edit to a prefix-resident file with `misses` sampled either
  side, the same shape as the Task 3.2 experiment.
