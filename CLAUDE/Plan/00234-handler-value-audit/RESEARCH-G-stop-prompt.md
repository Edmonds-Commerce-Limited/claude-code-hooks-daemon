...
7. **COST**: Bounded now (5 MiB cap, front-truncated), so no longer an unbounded-disk risk — but it is pure write cost (disk I/O + JSON serialization of the full hook_input) on every sub-agent completion, for a value nothing reads.
