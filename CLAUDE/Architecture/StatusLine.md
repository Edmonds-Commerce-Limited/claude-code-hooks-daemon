...
- **Effort level bars**: 5-segment bar (`▌▌▌▌▌`) over the tiers `low`/`medium`/`high`/`xhigh`/`max`, one active (orange) bar per tier position, remaining bars dim grey. Sourced primarily from the LIVE `hook_input["effort"]["level"]` field Claude Code sends on every status-line request — the level actually resolved for this session and model, and the only way to see a session-only level (`max`, or a level applied with `s` in the effort slider, neither of which Claude Code saves) or a pin (`/effort`, `--effort`, `CLAUDE_CODE_EFFORT_LEVEL`) that outranks the per-model level saved in settings.json. Falls back to `effortLevel` in settings.json (via the shared `settings_reader.read_claude_settings()`, mtime-cached) when the live field is absent, defaulting further to `"high"` (Claude Code's own default on most models) for Claude 4+ models when neither is set. An unrecognized effort string renders as the `"high"` tier's bar count rather than crashing.
