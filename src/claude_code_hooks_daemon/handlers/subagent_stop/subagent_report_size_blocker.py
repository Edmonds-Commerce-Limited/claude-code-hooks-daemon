...
class SubagentReportSizeBlockerHandler(SubagentStopHandlerBase):
    ...
    def __init__(self) -> None:
        ...
        # Config flag, declared here so mypy can verify it and a typo in a
    ...
    def handle(self, hook_input: dict[str, Any]) -> BlockingResult:
        ...
        return BlockingResult(
            ...
            reason=(
                ...
                "Write the full report to a file instead: under the declared "
                "plan folder's `subagent-reports/{yymmdd}-{agent-name}-"
                "{model}.md` (or the configured fallback directory if this is "
                "not plan work), then reply with a short completion summary "
                "plus the file path — not the report content itself."
            ),
        )
    ...
    def get_claude_md(self) -> str | None:
        return (
            ...
            "folder's `subagent-reports/{yymmdd}-{agent-name}-{model}.md` (or "
            "the configured fallback directory for non-plan work), then reply "
            "with a short completion summary plus the file path."
        )
