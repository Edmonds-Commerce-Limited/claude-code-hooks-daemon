...
| `environment_indicator.py` | `EnvironmentIndicatorHandler` | 4        | Shows 💻 desktop / 🐳 docker / 📦 podman from the runtime `ProjectContext` cached at startup  |
...
| `settings_reader.py`    | mtime-cached reader for `~/.claude/settings.json` (shared by `ModelContextHandler` + `ThinkingModeHandler`, no double-parse) |
