...
class DocsQaSweepHandler(SessionStartHandlerBase):
    ...
    def get_claude_md(self) -> str | None:
        return (
            ...
            "`markdown_organization`'s write-time location gate). `test_dirs`\n"
            "always has a built-in cross-language fallback, so TEST-dir\n"
            "findings can fire even with no `layout:` block declared;\n"
            "`source_dirs` has no built-in, so SOURCE-dir findings stay\n"
            "dormant until a project declares `layout.source_dirs`. Lastly\n"
            "`unenforced-approval-gate` (a daemon-owned `<agent tree>/core/*.core.md`\n"
            "line that tells the agent to obtain a human's approval without\n"
            "naming, in the same paragraph, the daemon config key that enforces\n"
            "the gate — deployed guidance must match the daemon's intended\n"
            "workflow, so the fix is a real gate behind a named key, or no\n"
