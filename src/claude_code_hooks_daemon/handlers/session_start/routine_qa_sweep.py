...
class RoutineQaSweepHandler(SessionStartHandlerBase):
    ...
    def get_claude_md(self) -> str | None:
        ...
        return (
            ...
            "\n"
            "- `routine-never-run` — declared active and nothing on record has "
            "covered anything: no rows at all, or only skips and unfinished starts.\n"
            "- `routine-overdue` — past its period PLUS its grace. Only a run that "
            "COVERED ground counts; a start proves somebody began and a skip proves "
            "somebody decided not to, and neither reviewed an interval.\n"
