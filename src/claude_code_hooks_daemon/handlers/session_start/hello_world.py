...
class HelloWorldSessionStartHandler(Handler):
    ...
    def __init__(self, priority: int = Priority.HELLO_WORLD) -> None:
        ...
        super().__init__(
            handler_id=HandlerID.HELLO_WORLD_SESSION_START,
            priority=priority,
    ...
    def get_acceptance_tests(self) -> list[Any]:
        ...
        return [
            AcceptanceTest(
                ...
                command='echo "test"',
                description="Test handler that confirms SessionStart hook is active",
                expected_decision=Decision.ALLOW,
                expected_message_patterns=[r"SessionStart hook system active"],
                safety_notes="Test handler only - always allows with context message",
                test_type=TestType.CONTEXT,
                requires_event="SessionStart event (cannot be triggered by subagent)",
            ),
        ]
