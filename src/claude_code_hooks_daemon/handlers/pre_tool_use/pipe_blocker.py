...
class PipeBlockerHandler(PreToolUseHandlerBase):
    ...
    def get_claude_md(self) -> str | None:
        ...
        return (
            ...
            "anything else you are writing rather than executing. Where the "
            "REDIRECT sits makes no difference: `cat > notes.md <<'EOF'` and "
            "`cat <<'EOF' > notes.md` are the same command to bash and the same "
            "command here, and a delimiter carrying punctuation (`<<'EOF-1'`) "
            "counts too.\n\n"
