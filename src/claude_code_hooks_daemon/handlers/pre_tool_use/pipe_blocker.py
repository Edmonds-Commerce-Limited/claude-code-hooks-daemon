...
class PipeBlockerHandler(PreToolUseHandlerBase):
    ...
    def get_claude_md(self) -> str | None:
        ...
        return (
            ...
            "**A quoted-delimiter heredoc is exempt when NOTHING on its line can "
            "EXECUTE the body.** `cat >> notes.md <<'EOF' ... EOF` writes its body "
            "out verbatim — the shell expands nothing in it — so a `| tail` "
            "sitting in that body was never going to run, and blocking it would "
            "be wrong. Quote the delimiter (`<<'EOF'`) whenever the body is "
            "prose, a code snippet, or anything else you are writing rather than "
            "executing. Where the REDIRECT sits makes no difference: "
            "`cat > notes.md <<'EOF'` and `cat <<'EOF' > notes.md` are the same "
            "command to bash and the same command here, and a delimiter carrying "
            "punctuation (`<<'EOF-1'`) counts too.\n\n"
            "**But `bash <<'EOF'` IS scanned, because bash EXECUTES the body.** "
            "The quoted delimiter governs only what the OUTER shell expands on "
            "the way in; it never stops the receiving command running the bytes. "
            "So the exemption is granted from an allowlist of commands that "
            "consume their input as data (`cat`, `tee`, `git`, `jq`, `grep`, …), "
            "and THREE things must all pass it: the receiver, every stage the "
            "body is piped on to (`cat <<'EOF' | bash` runs it), and the command "
            "not sitting in a substitution (`$(cat <<'EOF' … )` puts the body's "
            "text in command position). Anything else — `bash`, `sh`, `python3`, "
            "`ssh host`, or simply a name the list does not carry — has its body "
            "scanned like any other command. Withholding the exemption is the "
            "cheap error here: it costs a false positive, where granting one "
            "wrongly costs the guard entirely.\n\n"
