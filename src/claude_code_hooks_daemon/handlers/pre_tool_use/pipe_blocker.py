...
class PipeBlockerHandler(PreToolUseHandlerBase):
    ...
    def get_claude_md(self) -> str | None:
        ...
        return (
            ...
            "**A quoted-delimiter heredoc is exempt when its RECEIVER only "
            "READS the body.** `cat >> notes.md <<'EOF' ... EOF` writes its body "
            ...
            "So the exemption is granted by RECEIVER, from an allowlist of "
            "commands that consume their input as data (`cat`, `tee`, `git`, "
            "`jq`, `grep`, …). Anything else — `bash`, `sh`, `python3`, `ssh "
            "host`, or simply a name the list does not carry — has its body "
