...
class SedBlockerHandler(Handler):
    ...
    def get_claude_md(self) -> str | None:
        return (
            ...
            "**The rule is broader than 'no in-place editing' — read this before "
            "assuming a read-only `sed` is fine.**\n\n"
            "**Blocked**:\n"
            "- `sed` with a flag cluster containing `i`, `e` or `n` — so `sed -i` and "
            "`sed -e`, but ALSO `sed -n`, anywhere in the command. `sed -n '1,20p' file` "
            "prints to stdout and cannot write, and is blocked anyway: `-n` and `-i` "
            "differ by one character, and the block costs you nothing because `Read` "
            "with `offset`/`limit` does the same job\n"
            "- `sed` as a command HEAD — at the start, or after `;`, `&&` or `||` — "
            ...
            "**Allowed**:\n"
            "- `sed` as a PIPE STAGE (after a single `|`, no `i`/`e`/`n` flags) — but "
            "ONLY when a `grep` or an `echo` also appears somewhere in the command. "
            "That condition is the surprising part: `cat f | sed 's/x/y/' | grep z` is "
            "allowed, while `cat f | sed 's/x/y/' | wc -l` is DENIED, even though "
            "neither can modify a file\n"
