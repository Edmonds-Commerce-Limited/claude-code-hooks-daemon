...
class PlanNumberHelperHandler(PreToolUseHandlerBase):
    ...
    def get_claude_md(self) -> str | None:
        return (
            ...
            "existed on disk.\n\n"
            "**Check its count against one you did not get from it.** The report "
            "carries a `Checked N live plans.` line, and the agent can only reconcile "
            "that against its own enumeration — which is no check at all when the "
            "enumeration is what went wrong. Measured on this agent: 34, then 32, then "
            "17 plans reported for the same unchanged tree of 34. So state the number "
            "of plan folders in the plan root when you dispatch it (`mkplan.bash` "
            "prints it for you), and when the report's N disagrees, re-dispatch rather "
            "than act on the verdict — a scout that read a different tree has not "
