...
class PythonQaSuppressionBlocker(Handler):
    ...
    def __init__(self) -> None:
        super().__init__(
            handler_id=HandlerID.PYTHON_QA_SUPPRESSION,
    ...
    def matches(self, hook_input: dict[str, Any]) -> bool:
        ...
        if not any(file_path_lower.endswith(ext) for ext in PYTHON_CONFIG.extensions):
            return False
        ...
        for pattern in PYTHON_CONFIG.qa_forbidden_patterns:
    ...
    def handle(self, hook_input: dict[str, Any]) -> HookResult:
        ...
        resources_text = "\n".join(
            f"  - {tool}: {url}"
            for tool, url in zip(PYTHON_CONFIG.qa_tool_names, PYTHON_CONFIG.qa_tool_docs_urls)
        )

        return HookResult(
            decision=Decision.DENY,
            reason=(
                f"🚫 BLOCKED: {PYTHON_CONFIG.name} QA suppression comments are not allowed\n\n"
                f"File: {file_path}\n\n"
