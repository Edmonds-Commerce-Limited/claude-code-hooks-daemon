...
class DestructiveGitHandler(Handler):
    ...
    def get_claude_md(self) -> str | None:
        return (
            ...
            "**Deleting a branch has a sanctioned path — use it instead of asking.** "
            "`git branch -D` stays blocked, but `hooks-daemon delete-branch <name>...` "
            "does strictly MORE checking than that flag and needs no human:\n\n"
            "```\n"
            ...
            "```\n\n"
            "It refuses by default and deletes only what it can prove is recoverable — the "
            "branch is merged, or every commit is already upstream by patch-id, or every "
            "file version on it is byte-identical to one still reachable from `main`. A "
            "recovery bundle is written before deletion unless you pass `--no-bundle`. If "
            "it refuses, it names the files whose CONTENT exists nowhere else; read those "
            "before considering `--allow-unproven`, which also requires `--reason`. Note "
            "that `git branch -d` refuses every branch after a history rewrite (their "
            "commits stop being ancestors), which is exactly the gap this command fills.\n\n"
