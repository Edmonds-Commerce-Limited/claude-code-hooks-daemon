...
class DestructiveGitHandler(PreToolUseHandlerBase):
    ...
    def get_claude_md(self) -> str | None:
        return (
            ...
            "**A heredoc is judged by its RECEIVER, not by its delimiter.** "
            "`cat <<'EOF'` and `git commit -F - <<'EOF'` hand the body over as data, "
            "so it is prose. `bash <<'EOF'` EXECUTES it — the quoted delimiter only "
            "governs what the OUTER shell expands on the way in — so that body is "
            "scanned like any other command, and so is one fed to `sh`, `python3`, "
            "`ssh host`, or any receiver not on the data-sink allowlist. Withholding "
            "the exemption costs a false positive; granting it wrongly cost five of "
