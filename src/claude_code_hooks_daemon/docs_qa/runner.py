"""Run the registered docs QA checks for one stage.

Mirrors :mod:`claude_code_hooks_daemon.plan_qa.runner`: the registry is
declarative data (:mod:`docs_qa.checks`), the checks are pure functions,
and this module is just the filter-and-accumulate loop between them.
"""

from claude_code_hooks_daemon.docs_qa.checks import all_checks
from claude_code_hooks_daemon.docs_qa.types import CheckContext, CheckSpec, CheckStage, Finding
