...
class ConfigTemplate:
    ...
    def generate_minimal() -> str:
        ...
        return (
            ...
            "  status_line: {}\n"
            "\n"
            "# Daemon plugins: custom handler modules (not Claude Code plugins)\n"
            "plugins:\n"
            "  paths: []\n"
            "  plugins: []\n"
        )
    ...
    def generate_full() -> str:
        ...
        return (
            ...
            "  # PostToolUse - After tool execution\n"
            "  post_tool_use:\n"
            "    git_hooks_executable_fixer: {enabled: true, priority: 26}  # Auto-fix non-executable git hooks\n"
            ...
            "    background_process_tracker: {enabled: true, priority: 27}  # Track backgrounded processes; advise harvest-background (never kills)\n"
            "    command_hints: {enabled: true, priority: 28}  # Config-driven advisory reminder after a configured command\n"
            "    recovery_cron_advisor: {enabled: true, priority: 29}    # Advise on failsafe recovery cron lifecycle (opt-out)\n"
            "    goal_injection: {enabled: false, priority: 30}  # Write <session>.goal-intent for the ccy supervisor on plan flip to In Progress (opt-in). Ledger 00466 RV6-M1: MUST stay below markdown_table_formatter's priority\n"
            "    markdown_table_formatter: {enabled: true, priority: 31}  # Auto-format markdown tables via mdformat. Ledger 00466 RV6-M1: MUST stay above goal_injection's priority\n"
            ...
            "  # PostToolUseFailure - After a tool call fails\n"
            "  post_tool_use_failure:\n"
            "    agent_terminated_early_failure_detector: {enabled: true, priority: 10}  # A foreground Agent/Task dispatch killed by a usage limit surfaces here (its error field), never on PostToolUse (opt-out)\n"
            "\n"
            "  # PermissionRequest - Auto-approve decisions\n"
            "  permission_request:\n"
            ...
            "  # (a greater priority number) runs (Plan 00237/00242) — put message-auditing\n"
            "  # handlers on nitpick. cron_stop_enforcer sits BELOW it (priority 7)\n"
            ...
            "  # Task 3.1 adds the first live handler back; Plan 00416 adds the\n"
            "  # SubagentStop twin of cron_stop_enforcer, sitting BELOW\n"
            "  # subagent_report_size_blocker for the same shadowing reason.\n"
            "  subagent_stop:\n"
            ...
            "    subagent_report_persistence: {enabled: true, priority: 10}  # Sensor: auto-save every sub-agent's final reply to a gitignored file, before the terminal size blocker below looks it up\n"
