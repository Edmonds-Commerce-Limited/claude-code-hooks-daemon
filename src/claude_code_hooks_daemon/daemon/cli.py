...
def _is_nested_git_repo_root(directory: Path) -> bool:
    """Whether ``directory`` is itself a git repository root.

    A normal checkout has a ``.git`` DIRECTORY; a worktree has a ``.git``
    FILE instead (pointing at the main checkout's worktree metadata under
    ``.git/worktrees/``). Either marks ``directory`` as a repository
    boundary the walk must not cross.
    """
...
def _iter_markdown_candidates(
    ...
    - Any directory below ``root`` that is itself a git repository is
      ...
      repository is deliberately not checked here — only a NESTED one is a
