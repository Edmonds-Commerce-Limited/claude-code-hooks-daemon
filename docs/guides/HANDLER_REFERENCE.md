...
**Effort is not this signal's business at all (Plan 00466 N47 review 2).** The supervisor holds no effort opinion and injects no `/effort` command of any kind — the per-model floor and the manual-effort latch described in earlier versions of this page are both gone. Claude Code SAVES every interactively-typed `/effort <level>` into your own `settings.json`, so any supervisor-typed level permanently overwrote your own saved choice, which was the exact fight this signal's restore mechanism exists to avoid for MODEL choice. Effort is `modelSettings` data you configure yourself; see `CLAUDE/development/CcySupervisor.md`.
