...
#### self_matching_process_probe

| Property       | Value                         |
| -------------- | ----------------------------- |
| **Config key** | `self_matching_process_probe` |
| **Priority**   | 17                            |
| **Type**       | Blocking                      |
| **Event**      | PreToolUse                    |

**Description:** Blocks a liveness probe whose literal pattern matches the argv of the shell running it. The Bash tool runs every command through `bash -c "<command>"`, so the pattern searched for is always on the searching shell's own command line — `pgrep -f` finds that shell every time and reports "still running" for ever. A reported incident lost a whole night to exactly this, and two hand-run checks with the same pattern confirmed the wrong answer.

**Blocked** — the pattern matches this command's own argv:

- `pgrep -f "<literal>"`, `pgrep -af '<literal>'`
- `pkill -f <literal>`, `pgrep -f <literal> | xargs kill` (both signal the shell that asked)
- `ps aux | grep "<literal>"` with no self-exclusion
...
- Better still, do not poll: a `run_in_background` task is tracked by the harness, which notifies on completion
- `pgrep -f '[p]rovision.bash'` — the bracket trick
- `pgrep -x <name>` — matches the process name, and the waiting shell is named `bash`
...
**Bash coverage is exhaustive, not illustrative.** Resolved via `get_bash_write_targets`: `>`, `>>`, `tee`, a heredoc, and `cp`/`mv`/`install`/`dd` destinations. Resolved by this handler's own extraction: `curl -o|--output`, `wget -O|--output-document`, `tar` creating an archive, `mkdir`, `rsync`/`scp` destinations, and any of these nested inside `sh -c "..."` / `bash -c "..."`.

**One gap remains, and it is not closable.** An interpreter one-liner such as `python3 -c "open('/tmp/x','w')"` is not caught, because resolving what it writes means executing it. A clean Bash command is therefore not evidence a write stayed in the repository, only that no recognised shape named a path outside it.

**Output flags are read per command, never generically.** `-o` does not mean "output file" everywhere — `grep -o` is only-matching and takes no argument, so a blind rule would read grep's *pattern* as a destination and deny a command that writes nothing. Likewise `tar -xf` extracts *from* a path and is treated as a read; only a create flag makes the archive a destination.

**The extraction is handler-local, deliberately.** `get_bash_write_targets` exists to answer "what content did this command author", which is why the content linters exclude `cp`/`mv` — a copy relocates bytes it did not write. Containment asks a different question: a file lands outside the repository just as thoroughly whether the command authored it or fetched it. Widening the shared accessor to serve this premise would change the verdicts of 22 dependent handlers.
...
**Turning it off disables the auto-restore.** Since the supervisor arms only on this signal, a session with no recorder never restores. That is the safe direction, and it is not silent: the supervisor writes `downgrade fable -> opus is unattributed (no fresh, unspent model_downgrade_recorder record) — no restore` to its decision log.

**Effort is not this signal's business at all (Plan 00466 N47).** The supervisor holds no effort opinion and injects no `/effort` command of any kind. Claude Code SAVES a level typed after `/effort` (`low` to `xhigh`; `max` applies to the session only) into your own `settings.json`, so any level the supervisor typed would overwrite your own saved choice — the same fight this signal's restore mechanism exists to avoid for MODEL choice. Effort is per-model `modelSettings` data you configure yourself; see `CLAUDE/development/CcySupervisor.md`.

**A record explains only a nearby drop, once.** The supervisor spends a record by its identity (`record_id`: the transcript entry's `uuid`, or its line's byte offset) once the episode it opened has recovered, and attributes a drop only when the downgrade the record describes happened within five minutes of the drop being seen. The recorder republishes the same record for the rest of the session, so without both a later pick of the fallback model you made yourself would match it again.
...
**Human-input blockage marker (Plan 00298, declaration token added by Plan 00337):** when the explicit-stop-explanation branch ALLOWs a stop declaring that the human is the only thing blocking it, the handler records a session-scoped marker (`human-input-blockage-marker.json` under the daemon's untracked dir). `failsafe_cron_blockage_suppressor` reads it to drop the next delivered failsafe-cron tick before it reaches the model. Fail-open: a missing `session_id` or unresolvable project context skips the write silently and never affects the Stop decision.

Two routes arm it, and only the first is the supported one:

- **The declaration token** — `STOPPING BECAUSE: [awaiting-human] <reason>`. Matched as a literal immediately after the prefix, so any wording after it works. Anchoring to that position is deliberate: matched anywhere in the text, a stop message merely *discussing* the token would arm suppression on a session that is not blocked, and the symptom would be ticks silently not arriving.
- **The legacy prose patterns** — a short enumerated set of exactly four ("blocked only on human input", "blocked only on the owner's input", "need user input", "waiting on the user's decision"), never a broad "input" substring match. These are a compatibility fallback and are **closed to extension**: a phrasing they miss is handled by the token, not by adding a fifth pattern. Two rounds of widening (Plan 00298 → Plan 00314) each ended in another miss, which is why the mechanism moved to an exact declaration.
