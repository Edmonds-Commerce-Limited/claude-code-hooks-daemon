...
class TestVendorChangesReminderHandler:
    ...
    def test_matches_vendor_git_add(self, bash_hook_input: HookInputFactory) -> None:
        hook_input = bash_hook_input("git add vendor/my-org/package/src/file.php")
        assert self.handler.matches(hook_input) is True

    def test_matches_vendor_git_commit(self, bash_hook_input: HookInputFactory) -> None:
        hook_input = bash_hook_input("git commit -m 'update' vendor/my-org/package/")
        assert self.handler.matches(hook_input) is True

    def test_no_match_non_vendor_git_add(self, bash_hook_input: HookInputFactory) -> None:
        hook_input = bash_hook_input("git add src/Entity/Order.php")
        assert self.handler.matches(hook_input) is False

    def test_no_match_non_git_command(self, bash_hook_input: HookInputFactory) -> None:
        hook_input = bash_hook_input("ls vendor/")
        assert self.handler.matches(hook_input) is False

    def test_no_match_write_tool(self, write_hook_input: HookInputFactory) -> None:
        hook_input = write_hook_input("vendor/my-org/package/file.php")
        assert self.handler.matches(hook_input) is False

    def test_handle_returns_advisory(self, bash_hook_input: HookInputFactory) -> None:
        hook_input = bash_hook_input("git add vendor/my-org/package/src/file.php")
