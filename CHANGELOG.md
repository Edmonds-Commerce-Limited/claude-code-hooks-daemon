...
This is a **minor release** that closes a silent fail-open on the observability axis for project-level handlers (Plan 00143): a loud "PROJECT PROTECTION DEGRADED" alert now fires at every session start (and surfaces through `status`/`health`/`check` and the upgrade script) whenever a `.claude/project-handlers/` handler fails to load, so broken protection is never mistaken for active protection. Also fixes a status-line dogfooding bug where the effort bars ignored session-only `/effort` overrides, retires the now-tautological `thinking_mode` status-line segment, and — found during this release's own acceptance testing gate — fixes a core bug where several ALLOW-decision advisory handlers were silently dropping their guidance instead of reaching the user, most notably a genuine gap in `HookResult`'s Stop/SubagentStop formatting that made 4 handlers complete no-ops in production.
...
- **`root_recursion_guard`'s own acceptance tests never exercised its real detection logic** — Found during this release's acceptance testing (Plan 00142, shipped in v3.29.0): 2 of the handler's 3 `get_acceptance_tests()` commands wrapped the dangerous scanner invocation in `echo "..."`, the safety idiom most other blocking handlers use for raw substring-matching detection. But this handler tokenizes the command with `shlex` and inspects the first real word of each shell segment, so `echo "find / ..."` makes `echo` the detected command instead of `find` — silently defeating detection and letting the acceptance tests report success without ever exercising the real code path. **Production detection was never affected** — `matches()`/`handle()` were always correctly covered by comprehensive unit tests (`test_matches_dangerous`/`test_does_not_match_safe`) that call the handler directly rather than through a shell-safety wrapper. Fixed the 3 acceptance test commands to use the `false && <real command>` short-circuit pattern (already used by `pipe_blocker`'s own tests) instead of `echo`, and added a regression test (`test_get_acceptance_tests_commands_actually_match_expected_decision`) that round-trips every handler's declared acceptance tests through `matches()` to catch this class of bug for this handler going forward.
- **`global_npm_advisor`'s own acceptance test asserted a message pattern that never appears in the real advisory** — `expected_message_patterns` included `"local install"`, but the actual `handle()` advisory text never contains that phrase. Fixed the pattern to `"npx"` (which the advisory does contain) and added a regression test (`test_get_acceptance_tests_message_patterns_actually_appear_in_handle_output`) that round-trips every declared acceptance test's message patterns through the real `handle()` output.
- **`HookResult`'s Stop/SubagentStop formatting silently dropped ALL non-blocking advisory context — a SEV-level fix affecting 4 handlers.** Per Claude Code's hooks documentation, Stop and SubagentStop accept `hookSpecificOutput.additionalContext` for non-blocking advisory feedback that continues the conversation, in addition to the top-level `decision`/`reason` blocking mechanism. `HookResult._format_stop_response()` never implemented this — it only ever emitted the top-level `decision`/`reason` fields for a DENY, and returned `{}` for any ALLOW decision regardless of `context`. This made the following handlers' advisory guidance a complete no-op in production, silently, for as long as they have existed: `task_completion_checker` (Stop), `hedging_language_detector` (Stop), `dismissive_language_detector` (Stop), and `remind_prompt_library` (Stop + SubagentStop, which also had its message in the wrong field — see below). Fixed `_format_stop_response()` to emit `hookSpecificOutput.additionalContext` whenever `context` is non-empty (regardless of decision), updated the Stop/SubagentStop JSON schemas (`core/response_schemas.py`) to allow the new field, and added regression tests (`test_to_json_stop_allow_with_context_surfaces_additional_context` and siblings in `test_hook_result.py`) that pin the documented contract.
- **Several ALLOW-decision advisory handlers put their user-facing guidance in `reason` instead of `context`, so it was silently dropped.** `HookResult.to_json()` only copies `reason` into the response for DENY/ASK decisions (its own docstring: "Explanation for deny/ask decisions") — an ALLOW decision's `reason` is dropped for PreToolUse, PostToolUse, Stop, and SubagentStop alike; only `context` (`additionalContext`) reaches the user for ALLOW. Found via a full audit prompted by discovering the `npm_command` instance below. Fixed by moving the affected content from `reason` to `context` (in some cases merging a previously-duplicated terse `context` summary with the fuller `reason` text that was actually being dropped), and added `to_json()` round-trip regression tests pinning the fix for each handler:
  - **`npm_command`** (PreToolUse) — the entire "consider creating `llm:` wrappers" advisory (shown to every project without `llm:` scripts in `package.json` — likely the most common case for new adopters) was silently lost.
  - **`lsp_enforcement`** (PreToolUse) — the LSP-tool guidance was lost both in `advisory` mode and on the "allowed after first block" (`block_once`) path.
  - **`task_tdd_advisor`** (PreToolUse) — the detailed RED/GREEN/REFACTOR TDD walkthrough was lost; only a terse 4-line context summary survived.
  - **`validate_eslint_on_write`** (PostToolUse) — the detailed `llm:lint` recommendation (example `package.json` script, full guide link) was lost; only a terse 3-line context summary survived.
  - **`lint_on_edit`** (PostToolUse) — the "lint check timed out" advisory was lost (its sibling "linter not installed" branch was already correct).
