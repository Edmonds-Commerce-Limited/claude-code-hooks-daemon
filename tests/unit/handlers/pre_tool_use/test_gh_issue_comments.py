...
class TestGhIssueCommentsHandler:
    ...
    def test_handle_scopes_the_comments_append_to_the_view_segment(
        ...
        sub-command (echo, not gh issue view) and producing a remediation
        that does not fix anything."""
        hook_input = {
            "tool_name": "Bash",
            "tool_input": {"command": "gh issue view 123 && echo done"},
        }
        result = handler.handle(hook_input)
        assert result.reason is not None
        assert "gh issue view 123 --comments && echo done" in result.reason
    ...
    def test_handle_does_not_borrow_a_json_flag_from_a_different_chained_command(
        ...
        be mistaken for the gh-issue-view invocation's own --json flag."""
        hook_input = {
            "tool_name": "Bash",
            "tool_input": {"command": "gh issue view 123 && echo --json foo"},
        }
        result = handler.handle(hook_input)
        assert result.reason is not None
        assert "gh issue view 123 --comments && echo --json foo" in result.reason
