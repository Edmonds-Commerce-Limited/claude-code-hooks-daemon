...
Every repo comes from the shared ``tmp_git_repo`` fixture (``tests/conftest.py``):
a local identity, signing off, and one committed ``tracked.txt``. Plan 00252
Task 2.3 retired the file-local ``_git_init``/``_give_identity`` helpers so
this file states no git premise of its own — the fixture is where the premise
lives, and the session-wide ``hermetic_git_environment`` is what makes a
...
def _nested_repo(outer: Path) -> Path:
    """Init a second repo INSIDE ``outer`` — the one shape the fixture cannot give.

    It only ever resolves paths, never commits, so no identity is needed.
    """
...
class TestResolveFor:
    def test_resolves_repo_for_path_inside(self, tmp_git_repo: Path) -> None:
        ...
        assert resolved.root.resolve() == tmp_git_repo.resolve()

    def test_resolves_for_target_that_does_not_exist_yet(self, tmp_git_repo: Path) -> None:
        (tmp_git_repo / "CLAUDE" / "Plan").mkdir(parents=True)
        target = tmp_git_repo / "CLAUDE" / "Plan" / "00042-not-created/PLAN.md"
        resolved = GitRepo.resolve_for(target)
        assert resolved is not None
        assert resolved.root.resolve() == tmp_git_repo.resolve()

    def test_resolves_nested_repo_not_outer(self, tmp_git_repo: Path) -> None:
        inner = _nested_repo(tmp_git_repo)
    ...
    def test_returns_none_when_git_unavailable(
        ...
        assert GitRepo.resolve_for(tmp_git_repo / "x.py") is None


class TestReadConfig:
    def test_absent_key_returns_none(self, tmp_git_repo: Path) -> None:
        repo = GitRepo(tmp_git_repo)
        assert repo.read_config(_KEY) is None

    def test_reads_written_value(self, tmp_git_repo: Path) -> None:
        repo = GitRepo(tmp_git_repo)
        repo.write_config(_KEY, "42")
        assert repo.read_config(_KEY) == "42"

    def test_returns_raw_string_not_parsed(self, tmp_git_repo: Path) -> None:
...
class TestWriteConfig:
    ...
    def test_overwrites_previous(self, tmp_git_repo: Path) -> None:
        repo = GitRepo(tmp_git_repo)
        repo.write_config(_KEY, "110")
    ...
    def test_value_survives_branch_switch(self, tmp_git_repo: Path) -> None:
        ...
        repo.write_config(_KEY, "110")
        subprocess.run(
            ["git", "-C", str(tmp_git_repo), "checkout", "-b", "feature"],
            capture_output=True,
            check=True,
            timeout=Timeout.GIT_CONTEXT,
        )
...
def _make_index_stale(repo: Path) -> None:
    """Touch the fixture's tracked file so git WANTS to refresh the index next read.
...
class TestReadOnlyGitDoesNotTakeTheIndexLock:
    ...
    def test_a_read_through_the_runner_does_not_rewrite_the_index(self, tmp_git_repo: Path) -> None:
        _make_index_stale(tmp_git_repo)
        before = _index_identity(tmp_git_repo)

        run_git(tmp_git_repo, "status", "--porcelain")

        assert _index_identity(tmp_git_repo) == before, (
    ...
    def test_the_control_shows_bare_git_would_have_rewritten_it(self, tmp_git_repo: Path) -> None:
        ...
        subprocess.run(
            ["git", "-C", str(tmp_git_repo), "status", "--porcelain"],
            capture_output=True,
            check=True,
            timeout=Timeout.GIT_CONTEXT,
        )
    ...
    def test_output_is_still_returned_verbatim(self, tmp_git_repo: Path) -> None:
        """Declining the refresh must not change what git reports."""
        (tmp_git_repo / "new.txt").write_text("x\n", encoding="utf-8")

        result = run_git(tmp_git_repo, "status", "--porcelain")
    ...
    def test_the_parent_environment_survives(self, tmp_git_repo: Path) -> None:
        ...
        """
        result = run_git(tmp_git_repo, "rev-parse", "--show-toplevel")

        assert result.returncode == 0, result.stderr
        assert result.stdout.strip() != ""

    def test_output_that_is_not_valid_utf8_is_reported_not_raised(self, tmp_git_repo: Path) -> None:
        ...
        """
        undecodable = tmp_git_repo / "bad.txt"
        undecodable.write_bytes(b"before \xff\xfe after\n")
        run_git(tmp_git_repo, "add", "bad.txt")
        committed = run_git(
            tmp_git_repo, "commit", "-m", "commit a file git cannot decode as UTF-8"
        )
        ...
        result = run_git(tmp_git_repo, "show", "HEAD:./bad.txt")

        assert result.returncode == 0, result.stderr
        assert "before" in result.stdout
        assert "after" in result.stdout

    def test_the_existing_read_helpers_route_through_it(self, tmp_git_repo: Path) -> None:
        ...
        GitRepo.resolve_for(tmp_git_repo / "src" / "x.py")
...
class TestEveryGitCallIsBounded:
    ...
    def test_the_runner_passes_a_timeout(self, tmp_git_repo: Path) -> None:
    ...
    def test_a_caller_may_tighten_the_timeout(self, tmp_git_repo: Path) -> None:
        """Hot paths (status line, per-prompt) want a shorter bound than startup."""
        with mock.patch("subprocess.run") as runner:
            runner.return_value = subprocess.CompletedProcess([], 0, "", "")
            run_git(tmp_git_repo, "status", timeout=Timeout.GIT_STATUS_SHORT)

        assert runner.call_args.kwargs["timeout"] == Timeout.GIT_STATUS_SHORT

    def test_a_timeout_is_reported_not_raised(self, tmp_git_repo: Path) -> None:
...
class TestCallerSuppliedEnvironment:
    ...
    def test_caller_variables_are_added_to_the_environment(self, tmp_git_repo: Path) -> None:
        with mock.patch("subprocess.run") as runner:
            runner.return_value = subprocess.CompletedProcess([], 0, "", "")
            run_git(tmp_git_repo, "fetch", env={"GIT_TERMINAL_PROMPT": "0"})

        passed = runner.call_args.kwargs["env"]
        assert passed["GIT_TERMINAL_PROMPT"] == "0"

    def test_caller_variables_do_not_replace_the_environment(self, tmp_git_repo: Path) -> None:
    ...
    def test_the_declined_index_lock_survives_a_caller_environment(
    ...
    def test_a_caller_cannot_re_enable_the_optional_lock(self, tmp_git_repo: Path) -> None:
        ...
        with mock.patch("subprocess.run") as runner:
            runner.return_value = subprocess.CompletedProcess([], 0, "", "")
            run_git(tmp_git_repo, "status", env={"GIT_OPTIONAL_LOCKS": "1"})
