...
class TestTheBlockSettlesTheConflictWithAHarnessInstruction:
    """A live harness instruction tells the agent to author file content with
    Bash and `sed`, which this daemon denies and which bypasses every content
    guard that runs before a write.

    Two agents in one session each had to notice the contradiction and
    adjudicate it alone; one classified the instruction as injected. Leaving
    each agent to re-derive the answer is how one of them eventually decides
    the other way. The block already explains the blind spot, so it is the
    right place to state which side wins, once, for every agent and subagent
    that reads CLAUDE.md.
    """

    def _block(self, tmp_path: Path) -> str:
        from claude_code_hooks_daemon.core.claude_md_injector import ClaudeMdInjector
        ...
        ClaudeMdInjector(
            workspace_root=tmp_path,
            handlers=[_StubHandler("pipe_blocker", "## Pipe Blocker\n\nX.")],
        ).inject()
        return claude_md.read_text(encoding="utf-8")

    def test_the_block_names_the_instruction_and_says_this_project_overrides_it(
        self, tmp_path: Path
    ) -> None:
        block = self._block(tmp_path)
        assert "outside this project" in block
        assert "does not override" in block

    def test_it_sits_with_the_blind_spot_it_is_about(self, tmp_path: Path) -> None:
