...
class TestMainFunction:
    ...
    @patch("claude_code_hooks_daemon.hooks.user_prompt_submit.ConfigLoader")
    @patch("claude_code_hooks_daemon.hooks.user_prompt_submit.FrontController")
    @patch("claude_code_hooks_daemon.hooks.user_prompt_submit.load_config_safe")
    ...
    def test_enable_tags_filters_handlers(
        ...
        config = {
            "handlers": {
                "user_prompt_submit": {
                    "enable_tags": ["context-injection"],
                    "git_context_injector": {"enabled": True},
                }
            },
            "daemon": {},
        }
        ...
        if len(register_calls) > 0:
            registered_handler = register_calls[0][0][0]
            assert any(tag in registered_handler.tags for tag in ["context-injection"])
    ...
    def test_disable_tags_filters_handlers(
        ...
        config = {
            "handlers": {
                "user_prompt_submit": {
                    "disable_tags": ["context-injection"],
                    "git_context_injector": {"enabled": True},
                }
            },
            "daemon": {},
