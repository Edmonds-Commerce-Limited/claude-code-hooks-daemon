"""Tests for Notification hook entry point."""

from pathlib import Path
from unittest.mock import Mock, patch

from claude_code_hooks_daemon.hooks.notification import (
...
class TestGetBuiltinHandlers:
    ...
    def test_returns_dictionary_of_handlers(self) -> None:
        ...
        assert len(handlers) == 1  # notification_logger

    def test_contains_notification_logger_handler(self) -> None:
        """Notification logger handler is present."""
        handlers = get_builtin_handlers()

        assert "notification_logger" in handlers
    ...
    def test_handler_classes_are_importable(self) -> None:
        ...
        assert handlers["notification_logger"] == NotificationLoggerHandler
...
class TestMainFunction:
    ...
    def test_creates_front_controller_with_correct_event(
        ...
        """FrontController created with Notification event name."""
        ...
        mock_fc_class.assert_called_once_with(event_name="Notification")
    ...
    def test_registers_enabled_builtin_handlers(
        ...
        config = {
            "handlers": {
                "notification": {
                    "notification_logger": {"enabled": True, "priority": 50},
                }
            },
            "daemon": {},
        }
        ...
        # Should register notification_logger
    ...
    def test_handlers_default_to_enabled_if_not_specified(
        ...
        config = {
            "handlers": {
                "notification": {
                    "notification_logger": {},  # No 'enabled' key
                }
            },
            "daemon": {},
        }
    ...
    def test_applies_custom_priority_from_config(
        ...
        config = {
            "handlers": {
                "notification": {
                    "notification_logger": {"enabled": True, "priority": 99},
                }
            },
            "daemon": {},
        }
    ...
    def test_hello_world_handler_registered_when_enabled(
        ...
        assert HelloWorldNotificationHandler in handler_types
    ...
    def test_hello_world_handler_not_registered_when_disabled(
        ...
        register_calls = mock_fc_instance.register.call_args_list
        handler_types = [type(call[0][0]) for call in register_calls]
        assert HelloWorldNotificationHandler not in handler_types
    ...
    def test_skips_disabled_handlers(self, mock_load_config: Mock, mock_fc_class: Mock) -> None:
        ...
        config = {
            "handlers": {
                "notification": {
                    "notification_logger": {"enabled": False},
                }
            },
            "daemon": {},
        }
    ...
    def test_tag_filtering_with_enable_tags(
        ...
        # notification_logger has tag "monitoring", should be registered
        register_calls = mock_fc_instance.register.call_args_list
        # Depends on handler tags, verify at least filtering logic worked
    ...
    def test_tag_filtering_with_disable_tags(
        ...
        # If notification_logger has "monitoring" tag, it should be skipped
        register_calls = mock_fc_instance.register.call_args_list
