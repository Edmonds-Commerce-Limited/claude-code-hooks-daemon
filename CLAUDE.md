...
| R-LSP-CONFIG-EXCLUDE               | a pyright config with no `exclude` for a tree that is not project code                                                                               | The language server reports other checkouts' and fixtures' defects against this one, and a noisy stream is skimmed                                                                                | Add the listed `exclude` entries (the advisory prints them ready to paste)                                               |
| R-LSP-SERVER-STALE                 | a running `pyright-langserver` older than the pyright config file                                                                                    | It is still analysing the scope the OLD config declared                                                                                                                                           | pkill -f '[p]yright-langserver' (the harness respawns it on the next LSP use)                                            |
