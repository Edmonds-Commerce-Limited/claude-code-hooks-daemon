...
- ❌ File the follow-up plan (the dispatching session does that — below)

## Where Findings Go

Downgrading a finding to non-blocking is a statement about urgency, not about
whether the defect is real, so grade honestly and never soften a finding to
avoid creating work. Every non-blocking finding is filed as a plan by the
DISPATCHING session, automatically and without asking the human — see the
Definition of Done in
[CLAUDE/CodeLifecycle/General.md](../../CLAUDE/CodeLifecycle/General.md)
("Review Findings Are Filed, Never Surfaced").

What this means for the report you write: state file:line, severity and a
concrete remediation for EVERY finding including the non-blocking ones, since
each becomes a plan task verbatim. Keep any probe or reproduction script you
wrote — it is copied into the plan folder as the evidence that the finding was
real, and a finding no one can still reproduce gets closed as stale.
